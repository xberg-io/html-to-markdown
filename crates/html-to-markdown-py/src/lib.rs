...
// alef:hash:4dc8e25a4fb0748c1ee8e693af160fca01c88ce61c2028c625cb32ef723e5877
...
pub fn convert(
    ...
    // Manually extract ConversionOptions to handle None correctly
    eprintln!("DEBUG: convert called with options={:?}", options.is_some());
    let mut options_core: Option<html_to_markdown_rs::ConversionOptions> = match options {
        None => {
            eprintln!("DEBUG: options is None");
            None
        }
        Some(py_options) => {
            eprintln!("DEBUG: options is Some, extracting");
            let binding_options: ConversionOptions = py_options.extract(py)?;
            eprintln!("DEBUG: extracted successfully");
            let json = serde_json::to_string(&binding_options)
                .map_err(|e| {
                    eprintln!("DEBUG: serde to_string failed: {}", e);
                    pyo3::exceptions::PyRuntimeError::new_err(e.to_string())
                })?;
            eprintln!("DEBUG: json serialized");
            let core_options = serde_json::from_str(&json)
                .map_err(|e| {
                    eprintln!("DEBUG: serde from_str failed: {}", e);
                    pyo3::exceptions::PyRuntimeError::new_err(e.to_string())
                })?;
            eprintln!("DEBUG: core options deserialized");
            Some(core_options)
        }
    };
