...
// alef:hash:7528fb57f776d3bad0076bff6836a710389ce97728e33afa18c7007fe7fcac39
...
pub fn convert(
    ...
    let options_core: Option<html_to_markdown_rs::ConversionOptions> = match options {
        Some(v) => {
            let json =
                serde_json::to_string(&v).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?;
            Some(serde_json::from_str(&json).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))?)
        }
        None => None,
    };
    let options_core: Option<html_to_markdown_rs::ConversionOptions> = if let Some(handle) = visitor_handle {
        let mut opts = options_core.unwrap_or_default();
