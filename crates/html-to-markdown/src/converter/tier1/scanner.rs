...
pub fn scan(
        ...
        match bytes[pos] {
            b'<' => {
                if text_start < pos {
                    ...
                    // Tier-2's `text_node.rs` pushes it verbatim when the next
                    // sibling isn't inline — see `flush_text`'s use of this flag for
                    // why that only matters (byte-for-byte) when the next sibling is
                    // specifically a nested `<ul>`/`<ol>`.
                    ...
                    // flushed sits directly in front of a following `<img>` — see
                    // `flush_text`'s use of `next_tag_is_img` alongside
                    // `Tier1State::last_emitted_was_img`.
                ...
                // element carrying `hidden` or a `style="display:none"` /
                // `style="visibility:hidden"` / `style="font-size:0"` declaration — tag
                // and all descendant content — as a raw-string pass Tier-2 always runs
                // before parsing,
                // before either tier is even aware of tag identity. Checked here,
                // before the `<svg>`/`<template>` special cases below, so it is as
                // tag-agnostic as the pass it mirrors (a hidden `<svg>` must bail
                // too — its dedicated branch below has no hidden-element awareness
                // of its own). Reuses that pass's exact helpers (widened to
                // `pub(crate)`) rather than re-implementing the `hidden`/style
                // declaration scan a third time — see the code-fence/code-span
                // duplication this lane already fixed above.
                let tag_open_end =
                ...
                // its content. Skip the entire subtree without emitting anything.
                // Self-closing form is rare but handled.
                // ~keep
                // ~keep Audit #12 follow-up: this comment previously cited
                // plain_text.rs SKIP_TAGS as the reason, which was true only for
                // the plain-text output path — the two tiers were silently
                // mismatched for Markdown output until main.rs's `"template" |
                // "noscript" => {}` arm landed (converter/main.rs, outside
                // tier1/). Both tiers now agree `<template>` is inert.
                ...
                // inline passthroughs, matching `TagKind::Inline` behaviour and
                // HTML's default inline rendering for unknown elements.  All
                ...
                if matches!(spec.kind, TagKind::Ignored) && spec.is_rawtext {
                    ...
                    // bytes are non-whitespace.  Two `<script>`/`<style>` tags sitting
                ...
                // `Table` is in the collect-set for `border`, which feeds the
                // `has_span && border="0"` leg of Tier-2's `looks_like_layout`.
                // Omitting it silently disables that bail: `attrs` is empty, so the
                // lookup returns None, `border_zero` stays false, and Tier-1 emits a
                // GFM table where Tier-2 emits a bullet list. `TableCell` was already
                // here for colspan, which is why only the border half was affected.
                // `Blockquote` is here for `cite`, and it fell into the same trap: the
                ...
                // link's label buffer is no longer flat text — Tier-2's autolink
                // predicate strips such tags before comparing to href, so
                // `close_link` cannot safely trust a literal buffer mismatch here.
                // Mark every currently open link (not just the innermost, in case
                // of malformed nested `<a>`) so `close_link` can bail via
                // `BailReason::LinkAutolinkNestedMarkup` instead of risking a false
                // negative against Tier-2's tag-stripped text.
                ...
                // `push_list_item_indent` hardcodes a uniform 2-space-per-depth
                // scheme. That is correct for ul-in-ul (all markers are 2 wide) but
                // wrong the moment an ordered list is anywhere in the ancestor chain
                // of a nested list. Bail so Tier-2's cumulative-width logic wins.
                ...
                if name_lower == b"abbr" {
                    ...
                    // (issue #494), so a raw read here would reintroduce the defect in
                    // the fast path alone and split the two tiers.
                ...
                // recursion — the scanner itself has no stack-overflow risk.
                // But Tier-2's recursive `walk_node` silently truncates once
                // `depth >= effective_max_depth` (main.rs), and this scanner
                // had no equivalent ceiling: a pathologically deep DOM would
                // scan to completion here while Tier-2 truncates, diverging
                // from the byte-equality contract. Bail so Tier-2's (truncated,
                // authoritative) output wins instead of silently accepting
                // input Tier-2 would reject part of. `state.stack.len()` (open
                // tags below this one) is the same `depth` Tier-2 checks before
                // dispatching this node.
    ...
    // `trim_line_end_whitespace` runs BEFORE `collapse_excess_blank_lines`.
    // Without this, a code span's delimiter padding space that lands right
    // before an embedded newline in `<code>` content (e.g. a leading
    // `\n`-starting body next to a trailing backtick run needing a pad)
    // survives in Tier-1 but is stripped by Tier-2 as ordinary
    // end-of-line trailing whitespace — this is a general per-line pass,
    // not something scoped to code spans specifically, so reuse the exact
    // Tier-2 helper rather than reimplementing it.
                ...
                && bytes[name_start..name_start + tag_name.len()].eq_ignore_ascii_case(tag_name)
            {
                ...
                if matches!(
                    ...
                    if depth == 0 {
                        ...
                        // return `None` (not `Some(len + 1)`, an out-of-bounds
                        // offset one past the last valid slice/index into `bytes`).
                        // Callers (`find_svg_close` via `.unwrap_or(bytes.len())`)
                        // already handle `None` by clamping to end-of-input.
...
fn emit_open(
    ...
    // window (see the field's doc comment on `Tier1State`).
    ...
    match spec.kind {
        ...
        TagKind::Strong => {
            ...
            // tag's own bit is applied (`apply_open_escape_ctx` runs after
            // `emit_open` returns) — i.e. "is there already a `<strong>`
            // ancestor" — matching Tier-2's `ctx.in_strong` check in
            // `handle_strong`, which likewise tests the context inherited
            // from ancestors before forcing it true for its own children.
            // Without this, `<strong><strong>x</strong></strong>` emits
            // `****x****`, which is not valid CommonMark strong emphasis.
            // Tier-2's `handle_strong` walks children with no markers under `in_code`,
            // which covers both a `<code>` span and a `<pre>` block. Mirror via EscapeCtx.
                ...
                && !state.escape_ctx.contains(EscapeCtx::PRE)
            {
                ...
                // `**` close (no intervening content) would form one longer
                // CommonMark delimiter run on reparse, not two independent strong
                // spans. Tier-2 merges this case (`merge_adjacent_emphasis`);
                // Tier-1 does not replicate the merge, so it bails instead.
        ...
        TagKind::Emphasis => {
            ...
            // `*` marker -- but a lone `*` must not match the second half of a `**`
            // (that is a `<strong>` close, an unrelated tag kind), hence excluding
            // `ends_with("**")` here.
            // `handle_emphasis` is likewise transparent under `in_code`.
        ...
        TagKind::Strikethrough => {
            ...
            if !state.escape_ctx.contains(EscapeCtx::CODE) && !state.escape_ctx.contains(EscapeCtx::PRE) {
                ...
                // close forms one longer run on reparse, not two spans. Tier-2 merges
                // this (`emit_wrapped_inline`); Tier-1 does not, so it bails.
                if state.cell_or_output_mut().ends_with("~~") {
                    return Err(BailReason::AdjacentInlineEmphasis);
                }
                state.cell_or_output_mut().push_str("~~");
        ...
        TagKind::Block => {
            if block_container_is_passthrough(name_lower) {
                ...
                // NOT special-case a lone trailing newline into pushing just one more.
                // That produces a transient run of 3 newlines when a
                // single-newline-terminated sibling (a list/table/`<hr>`) precedes this
                // div; ordinarily harmless, since the final `collapse_excess_blank_lines`
                // pass folds any 3+ run back down to exactly 2 — EXCEPT when the div's
                // own first child is a `<blockquote>`, whose entry logic
                // (`close_blockquote`) inspects the exact trailing state before that
                // collapse runs and treats "ends with \n\n" (true for the transient
                // 3-run too) as "pop one", landing on the blank line Tier-2 actually
...
fn emit_void(
    ...
    match spec.kind {
        ...
        TagKind::LineBreak => {
            ...
            //     `normalize_link_label` (utility/content.rs) now preserves this
            //     exact marker mid-label instead of collapsing it (CommonMark
            //     spec examples 642/643 — a hard break inside link text is legal
            //     and must survive a convert/render/convert round trip), trimming
            //     it away only if it ends up at the label's start/end — a break
            //     with nothing before it has no preceding line to break, so a
            //     leading `<br>` (`<a><br>bar</a>`) must produce `[bar]`, not
            //     `[  \nbar]`. Router (`router.rs`) bails Tier-1 whenever
            //     `newline_style` is not `Spaces`, so this literal is the only
            //     marker Tier-1 ever needs to match. No trim beforehand otherwise,
            //     matching the non-heading branch below and Tier-2's own
            //     `line_break.rs`, which does not trim here either — only the
            //     label's own start/end trimming (in `normalize_link_label` /
            //     `close_link` below) cleans up the ends.
            //   - Inside a table cell (not in a link): mirrors Tier-2's
            //     `emit_table_cell_break` (main_helpers.rs) — trim trailing
            //     spaces/tabs, then emit a literal `<br>` when `br_in_tables`
            //     is true, or collapse to a single space (guarded against a
            //     leading space on an empty cell) otherwise.  `newline_style`
            //     is never consulted inside a cell (issue #453, issue #454).
            //   - Inside a regular block (paragraph, div, etc.): `"  \n"`, with any
            //     trailing whitespace already in the buffer (e.g. a decoded
            //     `&nbsp;` folded to a plain space) trimmed first so the hard-break
            //     prefix is exactly two spaces, not two-plus-N.  Mirrors the
            //     table-cell branch's own `trim_trailing_whitespace` call below.
            // Guarded off when `EscapeCtx::CODE` is set (issue #487): an open code
            // SPAN or BLOCK inside this link must run through the code-aware
            // branches below instead, not this one. Pushing this branch's literal
            // "  \n" straight into a still-open code span's content buffer would
            // land ahead of `close_code`'s segment split, so `content.split('\n')`
            // on "A  \nB" would cut "A  " -- not "A" -- as the first segment, and
            // those two spaces end up trapped inside its closing backtick, wrong
            // regardless of the link. Deferring here still comes out right: the
            // code-span branch pushes the same "  \n" shape OUTSIDE the backticks
            // once `close_code` re-joins the split segments, and `close_link`'s
            // `normalize_link_label` then finds that already-correct marker in the
            // assembled label text exactly as it would for plain (non-code) content.
            ...
            if inside_link_frame {
                ...
                // `<br>` is real content -- `B<a href="H"><br>A</a>` renders as B, a
                ...
                && state.escape_ctx.contains(EscapeCtx::HEADING)
            {
                ...
                // carry a hard break at all, so a code SPAN nested inside one folds
                // straight to a space here rather than reaching `close_code` as a
                // splittable marker -- `close_heading`'s own blind whitespace fold
                // runs AFTER `close_code` has already wrapped the content in
                // backticks, so by then it can only collapse whitespace BETWEEN two
                // already-separate spans, not merge them back into one (issue #487).
                let dest = state.cell_or_output_mut();
                ...
                // `<pre>` is itself inside a table cell, `close_pre` intentionally
                ...
                // not also inside a heading) lands here too: neither a cell nor a
                // code span can carry a hard break, and this table-cell branch
                // already folds to a single space (or emits nothing under an empty
                // cell) before `close_code` ever sees a splittable byte -- the same
                // "fold before it can split" reasoning as the heading arm above,
                ...
                // point, not span content. Push a plain '\n' as an internal-only
                // split marker -- `close_code` below splits on it into one backtick
                // span per segment, joined by the "  \n" hard-break marker OUTSIDE
                // the backticks (Tier-1 only ever runs under `NewlineStyle::Spaces`;
                // the router bails to Tier-2 for `Backslash`, so that is the only
                // marker shape this scanner ever needs -- issue #487). This byte can
                // only have come from a real `<br>`: a source text node's own
        ...
        TagKind::Image => {
            ...
            // landed in commit 55699777d5, after this scanner's pinned baseline) falls
            // back to `data-src` / `data-lazy-src` / `data-original` / `data-srcset` /
            // `srcset` -- in that precedence order -- whenever `src` is empty/whitespace
            // or a `data:` URI (the standard lazy-loading placeholder pattern: the real
            // image URL sits in a `data-*` attribute until the element scrolls into
            // view). See `BailReason::ImageLazyLoadSrc`'s doc comment for why this bails
            // rather than reimplementing that precedence (including `srcset`'s
            // width/density-descriptor comparison) here. A plain `<img src="x.png">`
            ...
            // longer depends on whether html5ever repaired the document: `&#x22;`,
            // `&quot;` and a literal `"` all converge on `"`. The canonicalize/bail fork
            // that stood here existed only to mirror that dependency, and with it gone
            // Tier-1 both matches Tier-2 and stops bailing on entity-bearing attributes.
            ...
            if keep_as_markdown {
                ...
                // outside tier1/ — same class of bug as the SVG title / link label
                // fixes above): an unescaped `alt` lets `x](https://evil.example`
                // close the image label early and open a second,
                // attacker-controlled Markdown image/link. Only applied on this
                // (markdown-emitting) branch — the `!keep_as_markdown` branch below
                ...
                if let Some(title_bytes) = title {
                    ...
                    // survived undecoded -- so the missing escape was invisible. Full
                    // decoding surfaces it: `title="&#x22;t&#x22;"` emitted `""t""`,
                    // which closes the title early and is not the document's text.
                    ...
                    // rather than interpolated, so the two tiers cannot drift on
                    // `(`/`)` escaping or the angle-bracket wrapping a whitespace-
                    // bearing URL needs. Decoding (#494) made both reachable from an
                    // entity, and a swept parity check found all four spellings
                    // diverging here.
                    dest.push_str("![");
...
fn emit_close(
    ...
    match spec.kind {
        ...
        // Tier-2 `form/elements.rs`'s `handle_button`.  No leading separator on open.
...
fn close_inline_marker(state: &mut Tier1State, frame: &OpenTag, marker: &str) -> Result<(), BailReason> {
    ...
    if body_is_empty {
        ...
        // before `flush_text` dropped it) non-empty but entirely whitespace (e.g.
        // `<i> </i>`) is not the same as a genuinely empty one (`<i></i>`) --
        // Tier-2's `chomp_inline` preserves it as a single space OUTSIDE the
        // markers, while erasing the markers here (as the genuinely-empty case
        // correctly does) would silently drop that space. Only bail for
        // every marker this function serves: Strikethrough (`~~`) and Inserted (`==`)
        // now go through Tier-2's shared `emit_wrapped_inline`, which preserves that
        // separator exactly as strong/emphasis do, so they are no longer exempt.
    ...
    // because the trailing check below indexes `buf` at this offset again:
    // moving `leading_len` bytes from right after the open marker to right
    // before it shifts every later byte's position forward by `leading_len`
    // without changing the buffer's total length, so the ORIGINAL
    // `content_start` value no longer points at the start of the real
    // content — it now lands `marker.len()` bytes into whatever sits at
    // the old marker position, which is a char boundary only by
    // coincidence. Reusing the stale value panicked
    // ("byte index is not a char boundary") on `<em>\u{2003}x</em>`-shaped
    // input, where the 1-byte `*` marker and the 3-byte migrated space
    // don't line up.
    ...
    // trailing whitespace run (e.g. a decoded `&nbsp;` folded to a plain
    // space) is pushed OUTSIDE the closing marker instead of staying
    // inside it, matching Tier-2's `chomp_inline` suffix handling.
    let content_str = &buf[content_start..];
...
fn close_heading(state: &mut Tier1State, frame: &OpenTag, n: u8, is_implicit: bool) -> Result<(), BailReason> {
    ...
    // body — not just internal whitespace runs — before prefixing. A leading
    // whitespace character (a decoded `&nbsp;`, or one migrated out of an
    // `<em>`/`<strong>` open marker by `close_inline_marker`'s leading-migration
    // step) would otherwise double up against the "# " prefix's own trailing
    // space (`<h3>&nbsp;x</h3>` -> "###  x" instead of Tier-2's "### x").
    // Trailing side of the same `text.trim()`. The whitespace-normalizing block above
    // only runs when the body contains a newline, so a heading whose body simply ENDS
    // in whitespace never reached a trailing trim -- `<h4><img alt="cafe &nbsp;"></h4>`
    // kept the decoded U+00A0 where Tier-2 dropped it. Found by the generated-corpus
    // parity test once issue #494's decoding made `&nbsp;` reach this path as a real
    // whitespace character rather than the literal text `&nbsp;`.
...
fn close_blockquote(state: &mut Tier1State, frame: &OpenTag) {
    ...
    // plain space) sitting right before `</blockquote>` is trimmed here,
    // mirroring `close_inline_marker`'s and `<br>`'s own trailing-whitespace
    // trim — Tier-2 does not carry it into the quoted line.
    ...
    // child content — not just internal whitespace runs — before the "> "
    // per-line prefixing. A leading whitespace character (a decoded
    // `&nbsp;`, or one migrated out of an `<em>`/`<strong>` open marker by
    // `close_inline_marker`'s leading-migration step) would otherwise double
    // up against the prefix's own trailing space (`<blockquote>&nbsp;x</blockquote>`
    // -> ">  x" instead of Tier-2's "> x").
    ...
    if frame.prev_escape_ctx.contains(EscapeCtx::BLOCKQUOTE) {
        ...
        // a clean blank line above; collapse it back to one newline here,
        // mirroring Tier-2's nested-blockquote separator handling.
        ...
        // existing blank-line separator collapses to a single newline, but a
        // lone trailing newline (or none at all) is PROMOTED up to a blank
        // line. This is a literal function of the untouched pre-open tail —
        // see `open_blockquote` for why nothing runs there in this branch.
        if state.output.ends_with("\n\n") {
            state.output.pop();
...
fn close_pre(state: &mut Tier1State, frame: &OpenTag, options: &ConversionOptions) {
    ...
    match options.code_block_style {
        ...
        CodeBlockStyle::Backticks => {
            ...
            // inside the content, otherwise the fence terminates early and
            // corrupts the rest of the document (CommonMark 4.5). Mirrors
            // `handlers::code_block::format_code_block`'s Backticks branch.
            ...
            // ALL trailing newlines, not just one: Tier-2's `format_code_block`
            // (handlers/code_block.rs) closes the fenced branch with
            // `content.trim_end_matches('\n')`, unconditionally, regardless of
            // how many trailing newlines `handle_pre` reconstructed upstream.
            // A nested block element that itself forces a full blank-line
            // separator on close (e.g. `<blockquote>`, since its fix) leaves 2
            // trailing newlines here when it is the last child of `<pre>` --
            // stripping only one left a spurious blank line before the closing
            // fence that Tier-2 never emits.
            ...
            // Backticks/Tildes branch with `output.push_str("\n\n")` — a clean
            // blank-line terminator, not a single newline.  Matching that
            // precisely matters beyond the general case (where a following
            // sibling's own leading-separator logic papers over a single-vs-
            // double difference either way): `close_blockquote`'s top-level
            // branch inspects this exact trailing state to decide whether to
            // collapse or promote the separator, so leaving only one newline
            // here made a `<pre>` directly followed by a `<blockquote>`
            // diverge from Tier-2 (discovered via that fix; see tests).
            rendered.push_str("\n\n");
        }
...
fn close_link(state: &mut Tier1State, frame: &OpenTag, options: &ConversionOptions) -> Result<(), BailReason> {
    ...
    if let Some(href) = href {
        ...
        // the shared `escape_link_label` helper — same class of bug just
        // fixed for the SVG `<title>` label above): an unescaped `]` in
        // link text lets `<a href="/x">a] (https://evil.example) b</a>`
        // close the label early and open a second, attacker-controlled
        // Markdown link — inert HTML source turned into a live injection
        // by the unescaped label. Only applies when a `[...]` label is
        // actually being emitted (the `href`-less branch below emits the
        // text with no bracket wrapping at all, matching Tier-2's separate
        // no-`escape_link_label` code path for that case). Applied after
        // the caret rewrite above (matching Tier-2's order); `↑` contains
        // no bracket so the rewrite is a no-op for `escape_link_label`
        // either way.
        // `escaped_label` borrows from `dest` when unescaped (the common case), so it
        // cannot outlive the `truncate`/`push_str` below that mutably borrows `dest` --
        // only rewrite `dest` when escaping actually produced an owned copy.
        ...
        if let Some(title) = title {
            ...
            // `replace('"', "\\\"")` in `escape_markdown_title` in `inline/link.rs`).  The
            // backslash-escape branch of link.rs appears unreachable in
            // practice for the title attribute path on these fixtures.
            // Same escaper as Tier-2's `append_markdown_link`. This used to substitute
            // `&quot;` instead, described as "mirror the observed fixture behaviour" --
            // behaviour observed in a world where attributes were only half-decoded, so
            // a literal `"` in a title was rare enough that the divergence from Tier-2's
            // `\"` went unnoticed. Full decoding (issue #494) makes it reachable.
            ...
            // interpolated (mirrors the image path above): an entity-shaped `&` decoded
            // by #494's full-decode (issue #498), an unbalanced paren, and a
            // whitespace-bearing destination all need the same escaping/wrapping Tier-2
            // applies, and interpolating `href` raw made all three diverge here.
            dest.push_str("](");
...
fn close_table(
    ...
    if !ts.has_th && !has_caption {
        ...
        // comment above `TableLayoutProbe` and the block comment above this function).
        ...
        // counts one nesting level only, which is what the probe accumulates.
        ...
        // halves are mirrored exactly: span = attribute presence, border = the
        // literal value "0" (see open_table_cell / open_table).
...
fn flush_text(
    ...
    // "next scanner event" after a custom-element close for the purposes
    // of the boundary check below (see the field's doc comment).
    ...
    // was an <img>" half of the adjacent-images check below.
    let after_img = state.last_emitted_was_img;
    state.last_emitted_was_img = false;

    // ~keep Inside a table but outside a cell or caption: whitespace between
    // structural tags (`<table>...<tr>`, `<tr>...<td>`) is insignificant and
    // silently dropped here, matching Tier-2 (which processes only tag
    // children of a table and never walks a stray whitespace text node).
    // Caption content is the exception — Tier-2 walks caption children and
    // accumulates their text into the caption output.
    //
    // ~keep Non-whitespace text at this position is HTML5 "foster parented"
    // content: the real parsing algorithm relocates it to just before the
    // `<table>` element rather than dropping it. Tier-2's own DOM (built by
    // `tl`, not a full HTML5 tree-construction implementation) only
    // sometimes surfaces it there — empirically, it depends on adjacency to
    // a `<!--comment-->` in a way this single-pass byte scanner cannot
    // reproduce byte-for-byte without a second pass over the table's
    // contents. Bail instead of silently discarding real content (the
    // original defect here): Tier-2's fallback renders whatever the true
    // answer is. `raw.trim().is_empty()` is the same "is this
    // insignificant" test already used everywhere else in this function,
    // so a decoded whitespace-only entity run (e.g. `&nbsp;` alone) is
    // treated as non-whitespace and bails too — overly conservative, but
    // never wrong, and rare enough in practice to not matter.
    ...
    // any other whitespace-only-node disposition and takes absolute
    // priority: at document start, a whitespace-only text node is DROPPED
    // outright, even when the surrounding context (e.g. between two
    // `<img>` tags' emitted Markdown and a following inline element) would
    // otherwise call for a single separating space. This matters because
    // `<img>` — like any non-text content — never flips
    // `Tier1State::at_document_start`, so it stays true across leading
    // images the way Tier-2's flag stays true across a leading `<img>`
    // (image handling never touches `at_fresh_block_start` either).
    // Excluded from table cells and list items, matching Tier-2's
    // `!ctx.in_table_cell && !ctx.in_list_item` guard — those have their
    // own dedicated whitespace handling below. Also excluded inside a
    // heading: Tier-2's heading handler is a separate, allow-listed
    // divergence (flattens a heading's `<img>` to bare alt text instead
    // of Markdown image syntax) that assembles its line outside the
    // generic text-node/`at_fresh_block_start` pipeline this mirrors, so
    // it always keeps a real separating space after that alt text —
    // applying this rule there would additionally drop that space and
    // compound one known divergence with a new one.
    ...
    if !in_pre && (is_block_edge || document_start_drops_ws) && raw_is_whitespace {
        ...
        // this Strong/Emphasis frame's own content start, BEFORE it disappears —
        // `close_inline_marker` cannot tell a genuinely empty `<em></em>` apart from
        // a whitespace-only `<em> </em>` from buffer bytes alone once this drop has
        // already erased them (see `OpenTag::dropped_whitespace_only_text`'s doc
        // comment). Harmless when more real content follows this same drop (e.g.
        // `<em> <b>x</b></em>`): the frame's body ends up non-empty regardless, so
        // `close_inline_marker` never consults the flag in that case.
        if at_inline_frame_start {
            if let Some(frame) = state.stack.last_mut() {
                ...
                // shared `emit_wrapped_inline` started preserving the separator (and, for
                // Code, the span) that a whitespace-only body stands for.
                if matches!(
                    frame.spec.kind,
    ...
    if !in_pre && !state.in_table_cell() && raw_is_whitespace {
        ...
        if !inside_inline {
            ...
            // OUTRIGHT (not deduped to a space, DROPPED) when it sits directly
            // between two "empty inline" siblings that are both direct
            // children of the same `<p>` — scoped here to the `<img>`/`<img>`
            // case the allow-listed divergence was keyed on (`is_empty_inline_element`
            // also covers `br`/`hr`/`input`, but those combinations are
            // unverified and out of scope). `state.stack.last()` being
            // `Paragraph` mirrors walking the `<p>`'s OWN direct children —
            // an intervening wrapper tag would put something else on top.
            ...
            // content requirement below — see
            // `Tier1State::last_closed_custom_element`.
                ...
                || (after_custom_element_close && !active_tail.is_empty() && !active_tail.ends_with('\n'))
            {
                ...
                if next_tag_is_list && raw.contains('\n') {
                    ...
                    // sibling check above when the whitespace run itself
                    // contains a newline (`had_newlines`): with a non-inline
                    // next sibling (`<ul>`/`<ol>`), that branch drops the run
                    // entirely rather than collapsing or preserving it. Push
                    // nothing here to match.
                } else if next_tag_is_list {
                    // ~keep Tier-2's text_node.rs pushes a whitespace-only text node
                    // VERBATIM (not collapsed) whenever the next sibling isn't
                    // inline — see `previous_sibling_is_inline_tag(...) &&
                    // next_sibling_is_inline_tag(...)`, false here since `<ul>`/
                    // `<ol>` are block. That raw tail then feeds
                    // `add_list_leading_separator`'s literal `ends_with("* "|
                    // "- "|". ")` check (list/utils.rs), which only
                    // false-negatives (collides with a bullet marker, skipping
                    // the separator) when the run is EXACTLY one space —
                    // collapsing here to a single space unconditionally would
                    // manufacture that collision for runs that never had it.
                    dest.push_str(raw);
                } else {
                    dest.push(' ');
                }
    ...
    // (non-whitespace) content — every earlier branch above either
    // returns early or is gated on `raw_is_whitespace`. This is the one
    // true "does real content already precede this text node, anywhere
    // in the document" signal, mirroring Tier-2's
    // `Context::at_fresh_block_start` (see `Tier1State::at_document_start`'s
    // doc comment for why buffer emptiness can't stand in for it).
    ...
    // document is insignificant — mirrors Tier-2's `was_fresh_block_start`
    // exclusion of table cells and list items (which have their own
    // dedicated whitespace handling reached via `block_separator_after`
    // and the table-cell gate below). Also excluded inside a heading:
    // Tier-2's heading handler is a separate, allow-listed divergence
    // (flattens a heading's `<img>` to bare alt text instead of
    // Markdown image syntax) that assembles its line outside the
    // generic text-node/`at_fresh_block_start` pipeline this mirrors, so
    // it always keeps the real space between that alt text and
    // following prose — stripping it here would additionally drop that
    // space and compound one known divergence with a new one.
    ...
    //     marker — Tier-2's text-node `skip_prefix` logic does the same, OR
    //   - we're at the very start of the document (see
    //     `document_start_strip` above).
    ...
    // `<strong>`/`<em>` body specifically (not `<a>`/`<code>`, and not the
    // summary-accumulation kinds handled by `at_inline_frame_start`'s other
    // arm), Tier-2's `chomp_inline` (utility/content.rs) does not delete
    // that whitespace: it collapses the run to a single ASCII space that
    // `close_inline_marker` then migrates outside the opening marker.
    // Deleting it outright — correct for `<a>` (`normalize_link_label`
    // really does trim) and left as-is for `<code>` (fully verbatim,
    // handled separately) — would make `<em>&nbsp;x</em>` render `*x*`
    // instead of Tier-2's ` *x*`. Push one space into the buffer here so
    // `close_inline_marker`'s existing leading-migration block (added
    // alongside its trailing counterpart) has something to move.
    // At document start there is nothing to migrate the space onto —
    // Tier-2's `skip_prefix` drops the prefix outright rather than
    // collapsing it to a space (`<em>&nbsp;x</em>` alone renders `*x*`,
    // not ` *x*`), so `document_start_strip` suppresses the migration.
    ...
    // block_separator_after case (a cell has no block separators to
    // speak of). But Tier-2's `normalize_link_label` trims a link label's
    // leading whitespace unconditionally, in a cell or not — so an `<a>`
    // frame must bypass the gate, or `<td><a> x</a></td>` keeps the space
    // Tier-2 trims (`[ x](...)` instead of `[x](...)`).
    ...
    // path). A bare inline element (`<span>`, `<u>`, `<mark>`, ... -- `TagKind::Inline`) has
    // none of those; its children flow through the exact same generic per-text-node path as
    // top-level prose, so Tier-2's `skip_prefix` (text_node.rs) only drops THIS text node's
    // leading whitespace when its own specific condition holds: `output.ends_with(' ') &&
    // prefix == " " && !previous_sibling_is_inline_tag(...)`. This text node is the very
    // first content written into the current frame (the same "nothing emitted yet" proxy
    // `at_inline_frame_start` uses for "no previous sibling"), so that last clause is
    // trivially true here; only `output.ends_with(' ')` needs checking. `prefix == " "` is
    // implicit in the trim below only firing when `raw` actually starts with ASCII
    // whitespace, which is exactly when Tier-2's `chomp` produces `prefix == " "`.
    // Confirmed empirically against Tier-2: `<span>with </span>\n  <span> more</span>` (a
    // whitespace-only sibling between the spans already pushed a separating space) collapses
    // the second span's own leading space away, but `<span>a</span><span> baz</span>` (no
    // separating whitespace, so `output` does not already end in a space) keeps it verbatim
    // (Google Docs' pretty-printed multi-`<span>` export hits the first shape: issue found
    // via `test_documents/html/office-gdocs/gdocs-web-page-export.html`). `<del>`/`<ins>`
    // (Strikethrough/Inserted) do NOT get this treatment -- Tier-2's dedicated strike/ins
    // wrapper leaves a genuine double space in the equivalent shape (verified) -- so they
    ...
    // trimming, no whitespace normalization at all. `in_code` (already
    // computed above) covers both bare `<code>` and `<pre><code>`; the
    // `<pre>` case is already excluded via `!in_pre`, so this only adds
    // the bare-`<code>` exclusion needed to stop a leading plain ASCII
    // space/tab/newline run from being deleted (`<code> x</code>` must
    // stay "` x`", not become "`x`").
    let raw = if !in_pre
        && !in_code
    ...
    if in_code {
        ...
        // code SPAN folds any raw line ending -- CR, LF, or CRLF -- in its own text
        // content to a single space; only a `<br>` element (handled separately by
        // `emit_void`'s `TagKind::LineBreak` arm) produces the literal '\n'
        // `close_code` treats as a split point between segments. A text node's own
        // line ending is never that marker, so folding it here can never collide
    ...
    // `\n  \n  ` between text runs.
    // Table cells fold newlines the same way: Tier-2's `process_text_node`
    // has a dedicated `ctx.in_table_cell` branch (text_node.rs) that runs
    // `normalize_cell_whitespace_cow` per text node — folding `\n`/`\r` into
    // the whitespace run before collapsing to a single space — rather than
    // the generic non-cell chomp path.  Without this, a text node whose
    // trailing run is `\n` + indentation (pretty-printed HTML, e.g. a
    // multi-line `<td>` around an `<ins>`/`<span>`) leaves the `\n` byte
    // un-collapsed here; `close_table_cell`'s later blanket
    // `replace('\n', " ")` then turns that leftover `\n` into a SECOND
    // space next to the one already collapsed from the trailing run,
    // double-spacing the cell relative to Tier-2.
    ...
    //          → `trailing_single_newline_join(...)` if trailing run was `\n` only.
    ...
    // apply the same rule or the two tiers disagree on every document containing
    // one.  The rule runs over the bytes this call appends rather than over `raw`,
    ...
    // first and only then folds (`normalize_link_label` / the summary
    // accumulator).  A `\` that sat before a newline in the source is escaped by
    // Tier-2 and would not be by a pass reading the already-folded text, so escape
    // the decoded-but-unfolded text and let the collapse run over the result.
    // Entities are resolved up front, hence `has_entities: false` below — the
    // collapse pass must not decode a second time.
