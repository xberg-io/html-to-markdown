...
// alef:hash:6c0395926872a8fd7fed0190713aadbdeb8cc200e9a76830e07dd96fe355abf5
...
pub struct ConversionOptions {
    ...
    #[serde(default = "crate::serde_defaults::conversion_options_tier_strategy")]
    #[serde(skip_serializing_if = "String::is_empty")]
...
impl ConversionOptionsUpdate {
    ...
    pub fn set_preprocessing(&mut self, value: Option<&PreprocessingOptionsUpdate>) {
        self.preprocessing = value.map(|value| value.clone().into());
    }
...
impl ConversionResult {
    ...
    #[php(constructor)]
    pub fn new(
        tables: &ext_php_rs::types::ZendHashTable,
        warnings: &ext_php_rs::types::ZendHashTable,
        content: Option<String>,
        document: Option<&DocumentStructure>,
    ) -> PhpResult<Self> {
        let mut tables_core_result: Vec<TableData> = Vec::new();
        for (_, item) in tables.iter() {
            if let Some(parsed) = <&TableData as ext_php_rs::convert::FromZval>::from_zval(item) {
                tables_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to TableData".to_string(),
                ));
            }
        }
        let tables_core: Vec<TableData> = tables_core_result;
        let mut warnings_core_result: Vec<ProcessingWarning> = Vec::new();
        for (_, item) in warnings.iter() {
            if let Some(parsed) = <&ProcessingWarning as ext_php_rs::convert::FromZval>::from_zval(item) {
                warnings_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to ProcessingWarning".to_string(),
                ));
            }
        }
        let warnings_core: Vec<ProcessingWarning> = warnings_core_result;
        Ok(Self {
            content,
            document: document.cloned(),
            metadata: Default::default(),
            tables: tables_core,
            warnings: warnings_core,
        })
    }
    ...
    pub fn set_document(&mut self, value: Option<&DocumentStructure>) {
        self.document = value.map(|value| value.clone().into());
    }
...
impl DocumentMetadata {
    ...
    pub fn new(
        keywords: Vec<String>,
        openGraph: HashMap<String, String>,
        twitterCard: HashMap<String, String>,
...
impl DocumentNode {
    ...
    pub fn new(
        ...
        content: &NodeContent,
        children: Vec<u32>,
        annotations: &ext_php_rs::types::ZendHashTable,
        parent: Option<u32>,
        attributes: Option<HashMap<String, String>>,
    ) -> PhpResult<Self> {
        let mut annotations_core_result: Vec<TextAnnotation> = Vec::new();
        for (_, item) in annotations.iter() {
            if let Some(parsed) = <&TextAnnotation as ext_php_rs::convert::FromZval>::from_zval(item) {
                annotations_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to TextAnnotation".to_string(),
                ));
            }
        }
        let annotations_core: Vec<TextAnnotation> = annotations_core_result;
        Ok(Self {
            id,
            content: content.clone(),
            parent,
            children,
            annotations: annotations_core,
            attributes,
        })
    }
...
impl DocumentStructure {
    ...
    #[php(constructor)]
    pub fn new(nodes: &ext_php_rs::types::ZendHashTable, sourceFormat: Option<String>) -> PhpResult<Self> {
        let mut nodes_core_result: Vec<DocumentNode> = Vec::new();
        for (_, item) in nodes.iter() {
            if let Some(parsed) = <&DocumentNode as ext_php_rs::convert::FromZval>::from_zval(item) {
                nodes_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to DocumentNode".to_string(),
                ));
            }
        }
        let nodes_core: Vec<DocumentNode> = nodes_core_result;
        Ok(Self {
            nodes: nodes_core,
...
impl HeaderMetadata {
    #[php(constructor)]
    pub fn new(level: u8, text: String, depth: i64, htmlOffset: i64, id: Option<String>) -> Self {
        Self {
            level,
            text,
            id,
...
impl HtmlMetadata {
    ...
    pub fn new(
        ...
        headers: &ext_php_rs::types::ZendHashTable,
        links: &ext_php_rs::types::ZendHashTable,
        images: &ext_php_rs::types::ZendHashTable,
        structuredData: &ext_php_rs::types::ZendHashTable,
    ) -> PhpResult<Self> {
        let mut headers_core_result: Vec<HeaderMetadata> = Vec::new();
        for (_, item) in headers.iter() {
            if let Some(parsed) = <&HeaderMetadata as ext_php_rs::convert::FromZval>::from_zval(item) {
                headers_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to HeaderMetadata".to_string(),
                ));
            }
        }
        let headers_core: Vec<HeaderMetadata> = headers_core_result;
        let mut links_core_result: Vec<LinkMetadata> = Vec::new();
        for (_, item) in links.iter() {
            if let Some(parsed) = <&LinkMetadata as ext_php_rs::convert::FromZval>::from_zval(item) {
                links_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to LinkMetadata".to_string(),
                ));
            }
        }
        let links_core: Vec<LinkMetadata> = links_core_result;
        let mut images_core_result: Vec<ImageMetadata> = Vec::new();
        for (_, item) in images.iter() {
            if let Some(parsed) = <&ImageMetadata as ext_php_rs::convert::FromZval>::from_zval(item) {
                images_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to ImageMetadata".to_string(),
                ));
            }
        }
        let images_core: Vec<ImageMetadata> = images_core_result;
        let mut structuredData_core_result: Vec<StructuredData> = Vec::new();
        for (_, item) in structuredData.iter() {
            if let Some(parsed) = <&StructuredData as ext_php_rs::convert::FromZval>::from_zval(item) {
                structuredData_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to StructuredData".to_string(),
                ));
            }
        }
        let structuredData_core: Vec<StructuredData> = structuredData_core_result;
        Ok(Self {
            document: document.clone(),
            headers: headers_core,
            links: links_core,
            images: images_core,
            structured_data: structuredData_core,
        })
    }
...
impl ImageMetadata {
    ...
    pub fn set_dimensions(&mut self, value: Option<&ImageDimensions>) {
        self.dimensions = value.map(|value| value.clone().into());
    }
...
impl TableGrid {
    ...
    #[php(constructor)]
    pub fn new(rows: u32, cols: u32, cells: &ext_php_rs::types::ZendHashTable) -> PhpResult<Self> {
        let mut cells_core_result: Vec<GridCell> = Vec::new();
        for (_, item) in cells.iter() {
            if let Some(parsed) = <&GridCell as ext_php_rs::convert::FromZval>::from_zval(item) {
                cells_core_result.push(parsed.clone());
            } else {
                return Err(ext_php_rs::exception::PhpException::default(
                    "Failed to convert array element to GridCell".to_string(),
                ));
            }
        }
        let cells_core: Vec<GridCell> = cells_core_result;
        Ok(Self {
            rows,
            cols,
            cells: cells_core,
...
impl AnnotationKind {
    ...
    pub fn from_json(json: String) -> PhpResult<Self> {
        ...
        if !matches!(
            value.annotation_type_tag.as_str(),
            "bold"
                | "italic"
                | "underline"
                | "strikethrough"
            ...
            return Err(PhpException::default(format!(
                "Invalid AnnotationKind tag: {}",
                value.annotation_type_tag,
            )));
        }
        Ok(value)
    }
...
#[php_impl]
impl ImageType {
    pub const DATAURI: &str = "data_uri";
    pub const INLINESVG: &str = "inline_svg";
...
#[php_impl]
impl TextDirection {
    pub const LEFTTORIGHT: &str = "ltr";
    pub const RIGHTTOLEFT: &str = "rtl";
...
impl VisitResult {
    ...
    pub fn from_json(json: String) -> PhpResult<Self> {
        ...
        if !matches!(
            ...
            "continue" | "custom" | "skip" | "preserve_html" | "error"
        ) {
            return Err(PhpException::default(format!(
                "Invalid VisitResult tag: {}",
                value.type_tag,
            )));
        }
        Ok(value)
...
#[php_impl]
impl WarningKind {
    pub const IMAGEEXTRACTIONFAILED: &str = "image_extraction_failed";
    pub const ENCODINGFALLBACK: &str = "encoding_fallback";
    pub const TRUNCATEDINPUT: &str = "truncated_input";
    pub const MALFORMEDHTML: &str = "malformed_html";
    pub const SANITIZATIONAPPLIED: &str = "sanitization_applied";
    pub const DEPTHLIMITEXCEEDED: &str = "depth_limit_exceeded";
}
...
impl From<ConversionOptions> for html_to_markdown_rs::options::ConversionOptions {
    fn from(val: ConversionOptions) -> Self {
        ...
        "invalid value {other:?} for field 'heading_style' of HeadingStyle: expected one of [Underlined, Atx, AtxClosed]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
            ...
            list_indent_type: match val.list_indent_type.as_str() {
                ...
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'list_indent_type' of ListIndentType: expected one of [Spaces, Tabs]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
            ...
            highlight_style: match val.highlight_style.as_str() {
                ...
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'highlight_style' of HighlightStyle: expected one of [DoubleEqual, Html, Bold, None]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
            ...
            whitespace_mode: match val.whitespace_mode.as_str() {
                ...
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'whitespace_mode' of WhitespaceMode: expected one of [Normalized, Strict]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
            ...
            newline_style: match val.newline_style.as_str() {
                ...
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'newline_style' of NewlineStyle: expected one of [Spaces, Backslash]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
            ...
            code_block_style: match val.code_block_style.as_str() {
                ...
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'code_block_style' of CodeBlockStyle: expected one of [Indented, Backticks, Tildes]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
            ...
            url_escape_style: match val.url_escape_style.as_str() {
                ...
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'url_escape_style' of UrlEscapeStyle: expected one of [Angle, Percent]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
            ...
            link_style: match val.link_style.as_str() {
                ...
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'link_style' of LinkStyle: expected one of [Inline, Reference]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
            ...
            output_format: match val.output_format.as_str() {
                ...
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'output_format' of OutputFormat: expected one of [Markdown, Djot, Plain]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
            ...
            tier_strategy: match val.tier_strategy.as_str() {
                "auto" => html_to_markdown_rs::options::TierStrategy::Auto,
                "tier2" => html_to_markdown_rs::options::TierStrategy::Tier2,
                "tier1" => html_to_markdown_rs::options::TierStrategy::Tier1,
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'tier_strategy' of TierStrategy: expected one of [auto, tier2, tier1]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
...
impl From<html_to_markdown_rs::options::ConversionOptionsUpdate> for ConversionOptionsUpdate {
    fn from(val: html_to_markdown_rs::options::ConversionOptionsUpdate) -> Self {
        Self {
            ...
            base_url: val.base_url.flatten().map(|v| v.to_string()),
            #[cfg(feature = "visitor")]
            visitor: val.visitor.map(|v| VisitorHandle {
                inner: std::sync::Arc::new(v),
            }),
    ...
    fn from(val: DocumentMetadata) -> Self {
        Self {
            ...
            text_direction: val.text_direction.as_deref().map(|s| match s {
                ...
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'text_direction' of TextDirection: expected one of [ltr, rtl, auto]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    html_to_markdown_rs::metadata::TextDirection::LeftToRight
                }
            }),
    ...
    fn from(val: ImageMetadata) -> Self {
        Self {
            ...
            image_type: match val.image_type.as_str() {
                ...
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'image_type' of ImageType: expected one of [data_uri, inline_svg, external, relative]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    html_to_markdown_rs::metadata::ImageType::DataUri
                }
            },
    ...
    fn from(val: LinkMetadata) -> Self {
        Self {
            ...
            link_type: match val.link_type.as_str() {
                ...
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'link_type' of LinkType: expected one of [anchor, internal, external, email, phone, other]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    html_to_markdown_rs::metadata::LinkType::Anchor
                }
            },
...
impl From<NodeContext> for html_to_markdown_rs::NodeContext<'_> {
    fn from(val: NodeContext) -> Self {
        html_to_markdown_rs::NodeContext::with_owned_attributes(
            serde_json::from_value(serde_json::Value::String(val.node_type.clone())).unwrap_or_else(|e| {
                let _ = ext_php_rs::exception::PhpException::default(format!("invalid NodeType: {e}")).throw();
                html_to_markdown_rs::NodeType::Text
            }),
            val.tag_name.into(),
    ...
    fn from(val: PreprocessingOptions) -> Self {
        Self {
            ...
            preset: match val.preset.as_str() {
                ...
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'preset' of PreprocessingPreset: expected one of [Minimal, Standard, Aggressive]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    Default::default()
    ...
    fn from(val: ProcessingWarning) -> Self {
        Self {
            ...
            kind: match val.kind.as_str() {
                ...
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'kind' of WarningKind: expected one of [image_extraction_failed, encoding_fallback, truncated_input, malformed_html, sanitization_applied, depth_limit_exceeded]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
    ...
    fn from(val: StructuredData) -> Self {
        Self {
            data_type: match val.data_type.as_str() {
                ...
                other => {
                    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'data_type' of StructuredDataType: expected one of [json_ld, microdata, rdfa]"
    ))
    .throw()
    .expect("failed to throw PHP exception for invalid enum string value");
                    html_to_markdown_rs::metadata::StructuredDataType::JsonLd
                }
            },
...
impl From<VisitResult> for html_to_markdown_rs::VisitResult {
    fn from(val: VisitResult) -> Self {
        match val.type_tag.as_str() {
            "continue" => html_to_markdown_rs::VisitResult::Continue,
            "custom" => html_to_markdown_rs::VisitResult::Custom(val.custom.unwrap_or_default()),
            "skip" => html_to_markdown_rs::VisitResult::Skip,
            "preserve_html" => html_to_markdown_rs::VisitResult::PreserveHtml,
            "error" => html_to_markdown_rs::VisitResult::Error(val.error.unwrap_or_default()),
            _ => html_to_markdown_rs::VisitResult::default(),
        }
    }
}
...
impl From<ConversionOptionsUpdate> for html_to_markdown_rs::options::ConversionOptionsUpdate {
    fn from(val: ConversionOptionsUpdate) -> Self {
        Self {
            heading_style: val.heading_style.as_deref().map(|s| match s { "Underlined" | "underlined" => html_to_markdown_rs::options::HeadingStyle::Underlined,"Atx" | "atx" => html_to_markdown_rs::options::HeadingStyle::Atx,"AtxClosed" | "atxclosed" => html_to_markdown_rs::options::HeadingStyle::AtxClosed,other => {
    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'heading_style' of HeadingStyle: expected one of [Underlined, Atx, AtxClosed]"
    ))
    .throw()
...
,
            list_indent_type: val.list_indent_type.as_deref().map(|s| match s { "Spaces" | "spaces" => html_to_markdown_rs::options::ListIndentType::Spaces,"Tabs" | "tabs" => html_to_markdown_rs::options::ListIndentType::Tabs,other => {
    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'list_indent_type' of ListIndentType: expected one of [Spaces, Tabs]"
    ))
    .throw()
...
,
            ...
            br_in_tables: val.br_in_tables,
            compact_tables: val.compact_tables,
            highlight_style: val.highlight_style.as_deref().map(|s| match s { "DoubleEqual" | "doubleequal" => html_to_markdown_rs::options::HighlightStyle::DoubleEqual,"Html" | "html" => html_to_markdown_rs::options::HighlightStyle::Html,"Bold" | "bold" => html_to_markdown_rs::options::HighlightStyle::Bold,"None" | "none" => html_to_markdown_rs::options::HighlightStyle::None,other => {
    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'highlight_style' of HighlightStyle: expected one of [DoubleEqual, Html, Bold, None]"
    ))
    .throw()
...
,
            extract_metadata: val.extract_metadata,
            whitespace_mode: val.whitespace_mode.as_deref().map(|s| match s { "Normalized" | "normalized" => html_to_markdown_rs::options::WhitespaceMode::Normalized,"Strict" | "strict" => html_to_markdown_rs::options::WhitespaceMode::Strict,other => {
    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'whitespace_mode' of WhitespaceMode: expected one of [Normalized, Strict]"
    ))
    .throw()
...
,
            ...
            sub_symbol: val.sub_symbol,
            sup_symbol: val.sup_symbol,
            newline_style: val.newline_style.as_deref().map(|s| match s { "Spaces" | "spaces" => html_to_markdown_rs::options::NewlineStyle::Spaces,"Backslash" | "backslash" => html_to_markdown_rs::options::NewlineStyle::Backslash,other => {
    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'newline_style' of NewlineStyle: expected one of [Spaces, Backslash]"
    ))
    .throw()
...
,
            code_block_style: val.code_block_style.as_deref().map(|s| match s { "Indented" | "indented" => html_to_markdown_rs::options::CodeBlockStyle::Indented,"Backticks" | "backticks" => html_to_markdown_rs::options::CodeBlockStyle::Backticks,"Tildes" | "tildes" => html_to_markdown_rs::options::CodeBlockStyle::Tildes,other => {
    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'code_block_style' of CodeBlockStyle: expected one of [Indented, Backticks, Tildes]"
    ))
    .throw()
...
,
            ...
            preserve_tags: val.preserve_tags.map(|v| v.into_iter().collect()),
            skip_images: val.skip_images,
            url_escape_style: val.url_escape_style.as_deref().map(|s| match s { "Angle" | "angle" => html_to_markdown_rs::options::UrlEscapeStyle::Angle,"Percent" | "percent" => html_to_markdown_rs::options::UrlEscapeStyle::Percent,other => {
    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'url_escape_style' of UrlEscapeStyle: expected one of [Angle, Percent]"
    ))
    .throw()
...
,
            link_style: val.link_style.as_deref().map(|s| match s { "Inline" | "inline" => html_to_markdown_rs::options::LinkStyle::Inline,"Reference" | "reference" => html_to_markdown_rs::options::LinkStyle::Reference,other => {
    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'link_style' of LinkStyle: expected one of [Inline, Reference]"
    ))
    .throw()
...
,
            output_format: val.output_format.as_deref().map(|s| match s { "Markdown" | "markdown" => html_to_markdown_rs::options::OutputFormat::Markdown,"Djot" | "djot" => html_to_markdown_rs::options::OutputFormat::Djot,"Plain" | "plain" => html_to_markdown_rs::options::OutputFormat::Plain,other => {
    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'output_format' of OutputFormat: expected one of [Markdown, Djot, Plain]"
    ))
    .throw()
...
,
            ...
            exclude_selectors: val.exclude_selectors.map(|v| v.into_iter().collect()),
            tier_strategy: val.tier_strategy.as_deref().map(|s| match s { "auto" => html_to_markdown_rs::options::TierStrategy::Auto,"tier2" => html_to_markdown_rs::options::TierStrategy::Tier2,"tier1" => html_to_markdown_rs::options::TierStrategy::Tier1,other => {
    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'tier_strategy' of TierStrategy: expected one of [auto, tier2, tier1]"
    ))
    .throw()
...
,
            base_url: (val.base_url).map(Some),
            visitor: val.visitor.map(|v| (*v.inner).clone()),
            ..Default::default()        }
    }
}
...
impl From<PreprocessingOptionsUpdate> for html_to_markdown_rs::options::PreprocessingOptionsUpdate {
    fn from(val: PreprocessingOptionsUpdate) -> Self {
        Self {
            enabled: val.enabled,
            preset: val.preset.as_deref().map(|s| match s { "Minimal" | "minimal" => html_to_markdown_rs::options::PreprocessingPreset::Minimal,"Standard" | "standard" => html_to_markdown_rs::options::PreprocessingPreset::Standard,"Aggressive" | "aggressive" => html_to_markdown_rs::options::PreprocessingPreset::Aggressive,other => {
    ext_php_rs::exception::PhpException::default(format!(
        "invalid value {other:?} for field 'preset' of PreprocessingPreset: expected one of [Minimal, Standard, Aggressive]"
    ))
    .throw()
...
mod serde_defaults {
    ...
    pub fn conversion_options_tier_strategy() -> String {
        "auto".to_string()
    }
...
pub extern "C" fn get_module() -> *mut ::ext_php_rs::zend::ModuleEntry {
    ...
    __EXT_PHP_RS_MODULE_ENTRY.get_or_init(|| {
        let builder = ::ext_php_rs::builders::ModuleBuilder::new("html_to_markdown", "3.15.0")
