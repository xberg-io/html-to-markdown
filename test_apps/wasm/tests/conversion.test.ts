// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:054a7268533aa3ea597bf158b3ab313b5626fae2a0ffeed86f2e16909698ddb5
// To regenerate: alef generate
// To verify freshness: alef verify

import { describe, expect, it } from "vitest";
import { convert, WasmConversionOptions, WasmPreprocessingOptions, WasmVisitorHandle } from "@xberg-io/html-to-markdown-wasm";

async function _alefE2eDecompressAndParseJson(response: Response): Promise<unknown> {
	const contentEncoding = response.headers.get("content-encoding");
	let bytes = new Uint8Array(await response.arrayBuffer());

	// The runtime's own fetch (undici on Node, and the browser equivalent) decodes
	// `content-encoding` transparently and leaves the header in place, so by the time the
	// body reaches here it is usually already plain text. Decoding unconditionally on the
	// header alone therefore fails ("incorrect header check") against a server that really
	// did compress. Decode only when the payload still carries the encoding: gzip announces
	// itself with the 0x1f8b magic pair; brotli has no magic, so it is attempted and the
	// already-decoded case falls back to the bytes as received.
	if (contentEncoding === "gzip" && bytes.length >= 2 && bytes[0] === 0x1f && bytes[1] === 0x8b) {
		const { gunzipSync } = await import("zlib");
		bytes = new Uint8Array(gunzipSync(bytes));
	} else if (contentEncoding === "br") {
		const { brotliDecompressSync } = await import("zlib");
		try {
			bytes = new Uint8Array(brotliDecompressSync(bytes));
		} catch {
			// Already decoded by the runtime.
		}
	}

	const text = new TextDecoder().decode(bytes);
	return JSON.parse(text);
}

function _alefE2eText(value: unknown): string {
	return value == null ? "" : String(value);
}

function _alefE2eItemTexts(item: unknown): string[] {
	if (item == null || typeof item !== "object") {
		return [_alefE2eText(item)];
	}
	const record = item as Record<string, unknown>;
	const itemsText = Array.isArray(record.items) ? record.items.map(_alefE2eText).join(" ") : "";
	return [_alefE2eText(item), _alefE2eText(record.kind), _alefE2eText(record.name), _alefE2eText(record.source), _alefE2eText(record.alias), _alefE2eText(record.text), _alefE2eText(record.signature), itemsText];
}

function _alefE2eFormatMetadataDisplay(fm: unknown): string {
	if (fm == null) return "";
	if (typeof fm !== "object") return String(fm);
	const record = fm as Record<string, unknown>;
	const formatType = record.format_type;

	// FormatMetadata is internally tagged, so serde merges the variant payload's own fields as
	// siblings of the tag rather than nesting them: { format_type: 'image', format: 'PNG', ... }.
	// Read `format` off the record directly. Deliberately NO nested (`record.image`) fallback --
	// a binding that regresses to the pre-flatten shape must fail this assertion loudly instead
	// of silently degrading to the variant name below.
	if (formatType === "image" && typeof record.format === "string") {
		return record.format;
	}

	// Fallback: return format_type variant name
	if (typeof record.format_type === "string") return record.format_type;
	return "";
}


describe("conversion", () => {

	it("adjacent_code_spans_merged: Adjacent code spans merge instead of colliding backticks", () => {
		const result = convert("<p><code>A</code><code>B</code></p>");
    expect((result.content ?? "")).toBe("`AB`\n");

	}, 30000);

	it("adjacent_emphasis_merged: Adjacent emphasis elements share one delimiter run", () => {
		const result = convert("<p><i>A</i><i>B</i><i>C</i></p>");
    expect((result.content ?? "")).toBe("*ABC*\n");

	}, 30000);

	it("adjacent_inserted_merged: Adjacent inserted elements share one delimiter run", () => {
		const result = convert("<p><ins>A</ins><ins>B</ins></p>");
    expect((result.content ?? "")).toBe("==AB==\n");

	}, 30000);

	it("adjacent_strikethrough_merged: Adjacent strikethrough elements share one delimiter run", () => {
		const result = convert("<p><del>A</del><del>B</del></p>");
    expect((result.content ?? "")).toBe("~~AB~~\n");

	}, 30000);

	it("adjacent_variable_merged: Adjacent variable elements share one delimiter run", () => {
		const result = convert("<p><var>A</var><var>B</var></p>");
    expect((result.content ?? "")).toBe("*AB*\n");

	}, 30000);

	it("blockquote_code_block_indentation_preserved: Code block inside a blockquote keeps its line indentation", () => {
		const result = convert("<blockquote><pre><code>line1\n    line2 indented</code></pre></blockquote>");
    expect((result.content ?? "")).toBe("> ```\n> line1\n>     line2 indented\n> ```\n");

	}, 30000);

	it("blockquote_multiple_paragraphs: Blockquote with multiple paragraphs has each paragraph prefixed", () => {
		const result = convert("<blockquote><p>First paragraph.</p><p>Second paragraph.</p></blockquote>");
    expect(result.content).toContain("> First paragraph.");
    expect(result.content).toContain("> Second paragraph.");

	}, 30000);

	it("blockquote_nested: Nested blockquote produces double-prefixed lines", () => {
		const result = convert("<blockquote><p>Outer quote.</p><blockquote><p>Inner quote.</p></blockquote></blockquote>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("Outer quote.");
    expect(result.content).toContain("Inner quote.");

	}, 30000);

	it("blockquote_nested_list_indentation_preserved: Nested list inside a blockquote keeps its continuation indentation", () => {
		const result = convert("<blockquote><ul><li>item a<ul><li>sub a1</li></ul></li></ul></blockquote>");
    expect((result.content ?? "")).toBe("> - item a\n>   * sub a1\n");

	}, 30000);

	it("blockquote_simple: Simple blockquote", () => {
		const result = convert("<blockquote><p>Quote text</p></blockquote>");
    expect(result.content).toContain("> Quote text");

	}, 30000);

	it("blockquote_text_then_paragraph_gets_blank_line: Bare text followed by a paragraph inside a blockquote gets a blank-line separator instead of merging into one line", () => {
		const result = convert("<blockquote>Just text, then <p>a paragraph</p></blockquote>");
    expect((result.content ?? "")).toBe("> Just text, then\n>\n> a paragraph\n");

	}, 30000);

	it("blockquote_with_list: Blockquote containing a list preserves list items inside quote", () => {
		const result = convert("<blockquote><p>Quote intro:</p><ul><li>Point one</li><li>Point two</li></ul></blockquote>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("Quote intro:");
    expect(result.content).toContain("Point one");
    expect(result.content).toContain("Point two");

	}, 30000);

	it("bold_and_italic: Nested bold and italic", () => {
		const result = convert("<p><strong><em>both</em></strong></p>");
    expect(result.content).toContain("***both***");

	}, 30000);

	it("bold_strong: Strong tag converts to bold", () => {
		const result = convert("<p><strong>bold</strong></p>");
    expect(result.content).toContain("**bold**");

	}, 30000);

	it("br_only_bold_wrapper_keeps_separator: A bold wrapper holding only a line break still separates the words on either side", () => {
		const result = convert("<b>Alpha</b><b><br></b><b>Beta</b>");
    expect((result.content ?? "")).toBe("**Alpha** **Beta**\n");

	}, 30000);

	it("br_only_emphasis_inside_empty_strong_keeps_separator: A line-break-only emphasis nested inside an otherwise empty strong wrapper still separates the words on either side", () => {
		const result = convert("<p><strong>Alpha</strong><strong><em><br></em></strong>Beta</p>");
    expect((result.content ?? "")).toBe("**Alpha** Beta\n");

	}, 30000);

	it("code_block: Code block with language preserves content", () => {
		const result = convert("<pre><code class=\"language-python\">print('hello')</code></pre>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("print('hello')");

	}, 30000);

	it("code_block_br_keeps_real_newline: A <br> inside a fenced code block stays a real newline, not a split span", () => {
		const result = convert("<pre><code>A<br>B</code></pre>");
    expect((result.content ?? "")).toBe("```\nA\nB\n```\n");

	}, 30000);

	it("code_block_no_language: Code block without a language class preserves content", () => {
		const result = convert("<pre><code>plain code here</code></pre>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("plain code here");

	}, 30000);

	it("code_inline_in_paragraph: Inline code element nested inside a paragraph", () => {
		const result = convert("<p>Call the <code>initialize()</code> method first.</p>");
    expect(result.content).toContain("`initialize()`");

	}, 30000);

	it("code_span_br_splits_into_hard_break: A <br> inside an inline code span splits the span and emits a hard break", () => {
		const result = convert("<p>X <code>A<br>B<br>C</code></p>");
    expect((result.content ?? "")).toBe("X `A`  \n`B`  \n`C`\n");

	}, 30000);

	it("code_span_literal_newline_folds_to_space: A literal line ending inside a code span folds to a space, per CommonMark", () => {
		const result = convert("<p>see <code>a\nb</code> here</p>");
    expect((result.content ?? "")).toBe("see `a b` here\n");

	}, 30000);

	it("code_span_suppresses_nested_inline_markers: Inline markers inside a code span stay literal content", () => {
		const result = convert("<p><code>a<em>b</em><ins>c</ins></code></p>");
    expect((result.content ?? "")).toBe("`abc`\n");

	}, 30000);

	it("code_with_backticks_in_content: Inline code containing backtick characters is properly escaped", () => {
		const result = convert("<p>Use <code>`backtick` here</code> carefully.</p>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("backtick");

	}, 30000);

	it("conversion_autolink_filename_not_autolinked: Bare filename href (no URI scheme) renders as inline link, not autolink (#397)", () => {
		const result = convert("<a href=\"foobar.png\">foobar.png</a>");
    expect((result.content ?? "")).toBe("[foobar.png](foobar.png)\n");

	}, 30000);

	it("conversion_autolink_https_url: Absolute https URL with matching text renders as autolink (#397)", () => {
		const result = convert("<a href=\"https://example.com\">https://example.com</a>");
    expect((result.content ?? "")).toBe("<https://example.com>\n");

	}, 30000);

	it("conversion_autolink_mailto: mailto: link with matching email text renders as autolink (#397)", () => {
		const result = convert("<a href=\"mailto:a@b.com\">a@b.com</a>");
    expect((result.content ?? "")).toBe("<a@b.com>\n");

	}, 30000);

	it("conversion_autolink_mixed_filename_and_url: Mixed filename + URL: only the URL becomes an autolink (#397)", () => {
		const result = convert("<a href=\"foobar.png\">foobar.png</a> <a href=\"https://www.heise.de\">https://www.heise.de</a>");
    expect((result.content ?? "")).toBe("[foobar.png](foobar.png) <https://www.heise.de>\n");

	}, 30000);

	it("conversion_autolink_relative_path_not_autolinked: Relative path href renders as inline link (#397)", () => {
		const result = convert("<a href=\"/docs/intro.html\">/docs/intro.html</a>");
    expect((result.content ?? "")).toBe("[/docs/intro.html](/docs/intro.html)\n");

	}, 30000);

	it("div_whitespace_only_span_does_not_indent_image: A whitespace-only span opening a div does not indent the image after it into a code block", () => {
		const result = convert("<p>P</p>\n<div>\n<span>    </span><img alt=\"A\" src=\"S\"/>\n</div>");
    expect((result.content ?? "")).toBe("P\n\n![A](S)\n");

	}, 30000);

	it("emphasis_mark_highlight: mark tag produces highlighted output", () => {
		const result = convert("<p><mark>highlighted</mark></p>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("highlighted");

	}, 30000);

	it("emphasis_strikethrough_del: del tag converts to GFM strikethrough", () => {
		const result = convert("<p><del>deleted text</del></p>");
    expect(result.content).toContain("~~deleted text~~");

	}, 30000);

	it("emphasis_strikethrough_s: s tag converts to GFM strikethrough", () => {
		const result = convert("<p><s>strikethrough</s></p>");
    expect(result.content).toContain("~~strikethrough~~");

	}, 30000);

	it("emphasis_subscript: sub tag content is preserved", () => {
		const result = convert("<p>H<sub>2</sub>O</p>");
    expect(result.content).toContain("H");
    expect(result.content).toContain("2");
    expect(result.content).toContain("O");

	}, 30000);

	it("emphasis_superscript: sup tag content is preserved", () => {
		const result = convert("<p>x<sup>2</sup></p>");
    expect(result.content).toContain("x");
    expect(result.content).toContain("2");

	}, 30000);

	it("emphasis_underline_u: u tag content is preserved in output", () => {
		const result = convert("<p><u>underlined</u></p>");
    expect(result.content).toContain("underlined");

	}, 30000);

	it("form_input_elements: Form input elements produce readable output without form mechanics", () => {
		const result = convert("<form><label for=\"name\">Name:</label><input type=\"text\" id=\"name\" placeholder=\"Enter name\"></form>", (() => { const _u0 = WasmConversionOptions.default(); _u0.preprocessing = (() => { const _u1 = WasmPreprocessingOptions.default(); _u1.removeForms = false; return _u1; })(); return _u0; })());
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("Name");

	}, 30000);

	it("form_select_options: Select element with options produces readable output", () => {
		const result = convert("<form><label>Color:</label><select><option value=\"red\">Red</option><option value=\"blue\" selected>Blue</option><option value=\"green\">Green</option></select></form>", (() => { const _u0 = WasmConversionOptions.default(); _u0.preprocessing = (() => { const _u1 = WasmPreprocessingOptions.default(); _u1.removeForms = false; return _u1; })(); return _u0; })());
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("Color");

	}, 30000);

	it("form_textarea: Textarea element produces readable output", () => {
		const result = convert("<form><label>Message:</label><textarea>Default text content</textarea></form>", (() => { const _u0 = WasmConversionOptions.default(); _u0.preprocessing = (() => { const _u1 = WasmPreprocessingOptions.default(); _u1.removeForms = false; return _u1; })(); return _u0; })());
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("Message");

	}, 30000);

	it("heading_br_only_nested_emphasis_keeps_separator: A line break inside nested emphasis in a heading still separates the words on either side", () => {
		const result = convert("<h2><strong>Alpha</strong><strong><em><br></em></strong>Beta</h2>");
    expect((result.content ?? "")).toBe("## **Alpha** Beta\n");

	}, 30000);

	it("heading_h1: H1 heading", () => {
		const result = convert("<h1>Heading 1</h1>");
    expect((result.content ?? "")).toBe("# Heading 1\n");

	}, 30000);

	it("heading_h2: H2 heading", () => {
		const result = convert("<h2>Heading 2</h2>");
    expect((result.content ?? "")).toBe("## Heading 2\n");

	}, 30000);

	it("heading_h3: H3 heading", () => {
		const result = convert("<h3>Heading 3</h3>");
    expect((result.content ?? "")).toBe("### Heading 3\n");

	}, 30000);

	it("heading_h4: H4 heading", () => {
		const result = convert("<h4>Heading 4</h4>");
    expect((result.content ?? "")).toBe("#### Heading 4\n");

	}, 30000);

	it("heading_h5: H5 heading", () => {
		const result = convert("<h5>Heading 5</h5>");
    expect((result.content ?? "")).toBe("##### Heading 5\n");

	}, 30000);

	it("heading_h6: H6 heading", () => {
		const result = convert("<h6>Heading 6</h6>");
    expect((result.content ?? "")).toBe("###### Heading 6\n");

	}, 30000);

	it("image_alt_unmatched_open_bracket_is_escaped: An unmatched [ in image alt text is escaped so the image survives", () => {
		const result = convert("<img alt=\"[A;B)\" src=\"S\">");
    expect((result.content ?? "")).toBe("![\\[A;B)](S)\n");

	}, 30000);

	it("image_figure_figcaption: Figure with figcaption preserves both image and caption", () => {
		const result = convert("<figure><img src=\"sunset.jpg\" alt=\"A sunset\"><figcaption>Beautiful sunset over the ocean</figcaption></figure>");
    expect(result.content).toContain("![A sunset](sunset.jpg)");
    expect(result.content).toContain("Beautiful sunset over the ocean");

	}, 30000);

	it("image_kept_inline_in_link: keep_inline_images_in naming 'a' keeps an image inside a link that also holds a block element", () => {
		const result = convert("<a href=\"https://example.com/file.pdf\"><img src=\"pdf.png\"><p>Download PDF</p></a>", (() => { const _u0 = WasmConversionOptions.default(); _u0.keepInlineImagesIn = ["a"]; return _u0; })());
    expect(result.content).toContain("![](pdf.png)");
    expect(result.content).toContain("Download PDF");
    expect(result.content).toContain("https://example.com/file.pdf");

	}, 30000);

	it("image_linked: Image inside an anchor produces a linked image", () => {
		const result = convert("<a href=\"https://example.com\"><img src=\"icon.png\" alt=\"Icon\"></a>");
    expect(result.content).toContain("![Icon](icon.png)");
    expect(result.content).toContain("https://example.com");

	}, 30000);

	it("image_multiline_alt_block_marker_is_escaped: A block marker on a continuation line of multiline alt text is escaped", () => {
		const result = convert("<img alt=\"Alpha\n-\nBeta\" src=\"image.png\">");
    expect(result.content).toContain("![Alpha\n\\-\nBeta](image.png)");

	}, 30000);

	it("image_no_alt: Image without alt text produces image markdown", () => {
		const result = convert("<img src=\"banner.jpg\">");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("banner.jpg");

	}, 30000);

	it("image_simple: Image with alt text", () => {
		const result = convert("<img src=\"photo.jpg\" alt=\"A photo\">");
    expect(result.content).toContain("![A photo](photo.jpg)");

	}, 30000);

	it("image_src_entity_shaped_ampersand_kept_literal: An & that would read as a character reference in an image destination stays literal", () => {
		const result = convert("<img src=\"?&amp;plus;\">");
    expect((result.content ?? "")).toBe("![](?&amp;plus;)\n");

	}, 30000);

	it("image_with_title: Image with title attribute includes title in output", () => {
		const result = convert("<img src=\"chart.png\" alt=\"Sales chart\" title=\"Q3 Sales\">");
    expect(result.content).toContain("![Sales chart](chart.png");
    expect(result.content).toContain("Q3 Sales");

	}, 30000);

	it("inline_code: Inline code", () => {
		const result = convert("<p>Use <code>console.log()</code> to debug</p>");
    expect(result.content).toContain("`console.log()`");

	}, 30000);

	it("italic_em: Em tag converts to italic", () => {
		const result = convert("<p><em>italic</em></p>");
    expect(result.content).toContain("*italic*");

	}, 30000);

	it("line_break_br_tag: Single br tag produces a line break in output", () => {
		const result = convert("<p>First line.<br>Second line.</p>");
    expect(result.content).toContain("First line.");
    expect(result.content).toContain("Second line.");

	}, 30000);

	it("line_break_hr_tag: hr tag produces a horizontal separator between content", () => {
		const result = convert("<p>Before rule.</p><hr><p>After rule.</p>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("Before rule.");
    expect(result.content).toContain("After rule.");

	}, 30000);

	it("line_break_multiple_br: Multiple consecutive br tags in sequence", () => {
		const result = convert("<p>Start.<br><br>End.</p>");
    expect(result.content).toContain("Start.");
    expect(result.content).toContain("End.");

	}, 30000);

	it("link_anchor_fragment: Fragment-only anchor link is preserved", () => {
		const result = convert("<a href=\"#section\">Jump to section</a>");
    expect(result.content).toContain("[Jump to section](#section)");

	}, 30000);

	it("link_empty_href: Link with empty href produces output with the link text", () => {
		const result = convert("<a href=\"\">No destination</a>");
    expect(result.content).toContain("No destination");

	}, 30000);

	it("link_href_entity_shaped_ampersand_kept_literal: An & that would read as a character reference in a link destination or title stays literal", () => {
		const result = convert("<a href=\"?&amp;plus;\" title=\"&amp;plus;\">T</a>");
    expect((result.content ?? "")).toBe("[T](?&amp;plus; \"&amp;plus;\")\n");

	}, 30000);

	it("link_image_inside: Image inside a link produces a linked image", () => {
		const result = convert("<a href=\"https://example.com\"><img src=\"logo.png\" alt=\"Logo\"></a>");
    expect(result.content).toContain("![Logo](logo.png)");
    expect(result.content).toContain("https://example.com");

	}, 30000);

	it("link_label_block_marker_is_escaped: A block marker on a continuation line of a link label is escaped", () => {
		const result = convert("<a href=\"https://example.com\">Alpha<br>-<br>Beta</a>");
    expect(result.content).toContain("[Alpha  \n\\-  \nBeta](https://example.com)");

	}, 30000);

	it("link_label_unmatched_open_bracket_is_escaped: An unmatched [ in a link label is escaped so the link survives", () => {
		const result = convert("<a href=\"S\">[A;B)</a>");
    expect((result.content ?? "")).toBe("[\\[A;B)](S)\n");

	}, 30000);

	it("link_leading_line_break_is_kept: A <br> just after <a> is kept as a hard line break in the label", () => {
		const result = convert("Beta<a href=\"https://example.com\"><br>Alpha</a>");
    expect(result.content).toContain("Beta[  \nAlpha](https://example.com)");

	}, 30000);

	it("link_mailto: Mailto link is preserved with mailto: scheme", () => {
		const result = convert("<a href=\"mailto:user@example.com\">Email us</a>");
    expect(result.content).toContain("mailto:user@example.com");

	}, 30000);

	it("link_simple: Simple link", () => {
		const result = convert("<a href=\"https://example.com\">Example</a>");
    expect(result.content).toContain("[Example](https://example.com)");

	}, 30000);

	it("link_split_by_block_child_is_not_duplicated: An anchor wrapping a block that holds another anchor records its destination once, not twice", () => {
		const result = convert("<a href=\"/o\"><div><a href=\"/i\">Inner</a></div></a>");
    expect((result.content ?? "")).toBe("[](/o)\n\n[Inner](/i)\n");

	}, 30000);

	it("link_split_drops_empty_authored_half: An anchor split around a block drops its empty half when the reconstructed half carries the text", () => {
		const result = convert("<a href=\"/o\"><div>Text<a href=\"/i\">Inner</a></div></a>");
    expect((result.content ?? "")).toBe("[Text](/o)[Inner](/i)\n");

	}, 30000);

	it("link_split_keeps_text_bearing_half: An anchor split around a block keeps the half that carries its text", () => {
		const result = convert("<a href=\"/o\">Text<div><a href=\"/i\">Inner</a></div></a>");
    expect((result.content ?? "")).toBe("[Text](/o)\n\n[Inner](/i)\n");

	}, 30000);

	it("link_trailing_line_break_is_kept: A <br> just before </a> is kept as a hard line break in the label", () => {
		const result = convert("<a href=\"https://example.com\">Alpha<br></a>Beta");
    expect(result.content).toContain("[Alpha  \n](https://example.com)Beta");

	}, 30000);

	it("link_with_bold_text: Link containing bold text preserves formatting", () => {
		const result = convert("<a href=\"https://example.com\"><strong>Bold link</strong></a>");
    expect(result.content).toContain("**Bold link**");
    expect(result.content).toContain("https://example.com");

	}, 30000);

	it("link_with_title: Link with title attribute", () => {
		const result = convert("<a href=\"https://example.com\" title=\"Example Site\">Example</a>");
    expect(result.content).toContain("[Example](https://example.com");
    expect(result.content).toContain("Example Site");

	}, 30000);

	it("link_wrapping_table_renders_table: An anchor wrapping a table renders the table as a block instead of an escaped link label", () => {
		const result = convert("<a href=\"https://example.com/outer\"><table><tr><td><a href=\"https://example.com/inner\">Inner</a></td></tr></table></a>");
    expect(result.content).toContain("[https://example.com/outer](https://example.com/outer)");
    expect(result.content).toContain("[Inner](https://example.com/inner)");
    expect(result.content).not.toContain("\\|");

	}, 30000);

	it("list_definition_dl: Definition list with dt and dd elements", () => {
		const result = convert("<dl><dt>Term One</dt><dd>Definition of term one.</dd><dt>Term Two</dt><dd>Definition of term two.</dd></dl>");
    expect(result.content).toContain("Term One");
    expect(result.content).toContain("Definition of term one.");
    expect(result.content).toContain("Term Two");
    expect(result.content).toContain("Definition of term two.");

	}, 30000);

	it("list_item_multiple_paragraphs: List item containing multiple paragraphs", () => {
		const result = convert("<ul><li><p>First paragraph in item.</p><p>Second paragraph in item.</p></li><li>Simple item</li></ul>");
    expect(result.content).toContain("First paragraph in item.");
    expect(result.content).toContain("Second paragraph in item.");
    expect(result.content).toContain("Simple item");

	}, 30000);

	it("list_mixed_nested: Mixed list: ordered list nested inside unordered list", () => {
		const result = convert("<ul><li>Item A<ol><li>Sub 1</li><li>Sub 2</li></ol></li><li>Item B</li></ul>");
    expect(result.content).toContain("Item A");
    expect(result.content).toContain("Sub 1");
    expect(result.content).toContain("Sub 2");
    expect(result.content).toContain("Item B");

	}, 30000);

	it("list_nested_ordered: Nested ordered list with two levels of depth", () => {
		const result = convert("<ol><li>Step 1<ol><li>Step 1a</li><li>Step 1b</li></ol></li><li>Step 2</li></ol>");
    expect(result.content).toContain("Step 1");
    expect(result.content).toContain("Step 1a");
    expect(result.content).toContain("Step 1b");
    expect(result.content).toContain("Step 2");

	}, 30000);

	it("list_nested_unordered: Nested unordered list with two levels of depth", () => {
		const result = convert("<ul><li>Parent A<ul><li>Child A1</li><li>Child A2</li></ul></li><li>Parent B</li></ul>");
    expect(result.content).toContain("Parent A");
    expect(result.content).toContain("Child A1");
    expect(result.content).toContain("Child A2");
    expect(result.content).toContain("Parent B");

	}, 30000);

	it("list_task_checkboxes: Task list with checked and unchecked checkboxes", () => {
		const result = convert("<ul><li><input type=\"checkbox\" checked> Done task</li><li><input type=\"checkbox\"> Pending task</li></ul>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("Done task");
    expect(result.content).toContain("Pending task");

	}, 30000);

	it("nested_newline_only_spans_after_emphasis_keep_separator: A newline inside two nested spans after an emphasis still separates the words on either side", () => {
		const result = convert("<p><i>Alpha</i><span><span>\n</span></span>Beta</p>");
    expect((result.content ?? "")).toBe("*Alpha* Beta\n");

	}, 30000);

	it("nested_newline_only_spans_keep_separator: A newline inside two nested spans still separates the words on either side", () => {
		const result = convert("<p>Alpha<span><span>\n</span></span>Beta</p>");
    expect((result.content ?? "")).toBe("Alpha Beta\n");

	}, 30000);

	it("ordered_list: Ordered list", () => {
		const result = convert("<ol><li>First</li><li>Second</li><li>Third</li></ol>");
    expect(result.content).toContain("1. First");
    expect(result.content).toContain("2. Second");
    expect(result.content).toContain("3. Third");

	}, 30000);

	it("paragraph_multiple: Multiple paragraphs are separated by a blank line", () => {
		const result = convert("<p>First paragraph.</p><p>Second paragraph.</p>");
    expect(result.content).toContain("First paragraph.");
    expect(result.content).toContain("Second paragraph.");

	}, 30000);

	it("paragraph_nested_divs: Text nested inside divs is extracted correctly", () => {
		const result = convert("<div><div><p>Nested text</p></div></div>");
    expect(result.content).toContain("Nested text");

	}, 30000);

	it("paragraph_simple: Simple paragraph converts to plain text", () => {
		const result = convert("<p>Hello World</p>");
    expect((result.content ?? "")).toBe("Hello World\n");

	}, 30000);

	it("paragraph_whitespace_only_span_separates_words: A newline-only inline wrapper still separates the words on either side", () => {
		const result = convert("Alpha<span style=\"white-space:pre\">\n</span>13");
    expect((result.content ?? "")).toBe("Alpha 13\n");

	}, 30000);

	it("paragraph_with_inline_formatting: Paragraph with bold, italic, and a link", () => {
		const result = convert("<p>This has <strong>bold</strong>, <em>italic</em>, and a <a href=\"https://example.com\">link</a>.</p>");
    expect(result.content).toContain("**bold**");
    expect(result.content).toContain("*italic*");
    expect(result.content).toContain("[link](https://example.com)");

	}, 30000);

	it("paragraph_with_line_breaks: Paragraph with br tags produces line breaks in output", () => {
		const result = convert("<p>Line one.<br>Line two.<br>Line three.</p>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("Line one.");
    expect(result.content).toContain("Line two.");
    expect(result.content).toContain("Line three.");

	}, 30000);

	it("semantic_abbr: Abbreviation element text is preserved", () => {
		const result = convert("<p>The <abbr title=\"World Wide Web\">WWW</abbr> is global.</p>");
    expect(result.content).toContain("WWW");

	}, 30000);

	it("semantic_article: Article element wrapping content preserves inner content", () => {
		const result = convert("<article><h2>Article Title</h2><p>Article body.</p></article>");
    expect(result.content).toContain("Article Title");
    expect(result.content).toContain("Article body.");

	}, 30000);

	it("semantic_definition_list: Definition list with term and description", () => {
		const result = convert("<dl><dt>HTML</dt><dd>HyperText Markup Language</dd><dt>CSS</dt><dd>Cascading Style Sheets</dd></dl>");
    expect(result.content).toContain("HTML");
    expect(result.content).toContain("HyperText Markup Language");
    expect(result.content).toContain("CSS");
    expect(result.content).toContain("Cascading Style Sheets");

	}, 30000);

	it("semantic_details_summary: Details and summary elements produce readable output", () => {
		const result = convert("<details><summary>Click to expand</summary><p>Hidden content here.</p></details>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("Click to expand");

	}, 30000);

	it("semantic_hr: Horizontal rule produces a separator in output", () => {
		const result = convert("<p>Above</p><hr><p>Below</p>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("Above");
    expect(result.content).toContain("Below");

	}, 30000);

	it("semantic_mark_highlight: Mark tag produces highlighted output", () => {
		const result = convert("<p>This is <mark>highlighted text</mark> in a sentence.</p>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("highlighted text");

	}, 30000);

	it("semantic_section_with_heading: Section element with heading preserves structure", () => {
		const result = convert("<section><h3>Section Heading</h3><p>Section content.</p></section>");
    expect(result.content).toContain("Section Heading");
    expect(result.content).toContain("Section content.");

	}, 30000);

	it("semantic_sub_superscript: Subscript and superscript elements are preserved in output", () => {
		const result = convert("<p>H<sub>2</sub>O and E=mc<sup>2</sup></p>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("H");
    expect(result.content).toContain("2");
    expect(result.content).toContain("O");
    expect(result.content).toContain("E=mc");

	}, 30000);

	it("simple_table: Simple table with header", () => {
		const result = convert("<table><thead><tr><th>Name</th><th>Age</th></tr></thead><tbody><tr><td>Alice</td><td>30</td></tr></tbody></table>");
    expect(result.content).toContain("Name");
    expect(result.content).toContain("Age");
    expect(result.content).toContain("Alice");
    expect(result.content).toContain("30");
    expect(result.content).toContain("|");
    expect(result.content).toContain("---");

	}, 30000);

	it("space_only_italic_inside_empty_strong_keeps_separator: A space-only italic nested inside an otherwise empty strong wrapper still separates the words on either side", () => {
		const result = convert("<p><strong>Alpha</strong><strong><i> </i></strong>Beta</p>");
    expect((result.content ?? "")).toBe("**Alpha** Beta\n");

	}, 30000);

	it("strike_renders_as_strikethrough: The deprecated strike tag renders as strikethrough", () => {
		const result = convert("<p><strike>gone</strike></p>");
    expect((result.content ?? "")).toBe("~~gone~~\n");

	}, 30000);

	it("table_cell_br_only_emphasis_keeps_separator: A line-break-only emphasis inside a table cell still separates the words on either side", () => {
		const result = convert("<table><tr><td><strong>Alpha</strong><em><br></em>Beta</td></tr></table>");
    expect((result.content ?? "")).toBe("| **Alpha** Beta |\n| -------------- |\n");

	}, 30000);

	it("table_content_after_unclosed_row_survives: Content following a table whose last row is never closed is still converted", () => {
		const result = convert("<table><tr><th>Before</th></tr><tr></table><p>Visible footer</p>");
    expect(result.content).toContain("| Before |");
    expect(result.content).toContain("| ------ |");
    expect(result.content).toContain("Visible footer");

	}, 30000);

	it("table_empty: Empty table produces no output or minimal output", () => {
		const result = convert("<table></table>");
    expect((result.content ?? "")).toBe("");

	}, 30000);

	it("table_layout_cell_paragraphs_stay_separated: Adjacent paragraphs in a layout-table cell are separated rather than concatenated into one run of text", () => {
		const result = convert("<table><tr><td><p><b>Alice Example</b></p><p><i>Customer Service</i></p></td><td>Logo</td></tr><tr><td>A</td><td>B</td><td>C</td></tr></table>", (() => { const _u0 = WasmConversionOptions.default(); _u0.compactTables = true; return _u0; })());
    expect(result.content).not.toContain("**Alice Example***Customer Service*");
    expect(result.content).toContain("Alice Example");
    expect(result.content).toContain("Customer Service");
    expect(result.content).toContain("Logo");

	}, 30000);

	it("table_layout_row_keeps_image: A layout-table row keeps an image as Markdown instead of degrading it to alt text", () => {
		const result = convert("<table><tr><td><img src=\"a.svg\" alt=\"A\"> <a href=\"/1\">x</a></td><td><a href=\"/2\">y</a></td></tr><tr><td><a href=\"/3\">z</a></td></tr></table>");
    expect((result.content ?? "")).toBe("- ![A](a.svg) [x](/1) [y](/2)\n- [z](/3)\n");

	}, 30000);

	it("table_nested_chain_not_misclassified_as_layout: A straight chain of one-nested-table-per-cell tables renders as pipe tables, not a broken layout list", () => {
		const result = convert("<table><tr><td><table><tr><td><table><tr><td>leaf</td></tr></table></td></tr></table></td></tr></table>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("leaf");
    expect(result.content).toContain("| ---");

	}, 30000);

	it("table_nested_in_single_cell_renders_separately: A table nested in a single-cell row renders as its own table", () => {
		const result = convert("<table><tr><th>Summary</th></tr><tr><td>Before<table><tr><th>Item</th><th>Amount</th></tr><tr><td>Alpha</td><td>10</td></tr></table></td></tr></table>");
    expect(result.content).toContain("| Item  | Amount |");
    expect(result.content).toContain("| Alpha | 10     |");
    expect(result.content).not.toContain("\\|");

	}, 30000);

	it("table_nested_in_wrapper_renders_separately: A nested table wrapped in a div still renders as its own table rather than raw pipes", () => {
		const result = convert("<table><tr><th>Summary</th></tr><tr><td>Before<div><table><tr><th>Item</th><th>Amount</th></tr><tr><td>Alpha</td><td>10</td></tr></table></div></td></tr></table>");
    expect(result.content).toContain("| Item  | Amount |");
    expect(result.content).toContain("| Alpha | 10     |");
    expect(result.content).toContain("Before");
    expect(result.content).not.toContain("\\|");

	}, 30000);

	it("table_no_thead: Table without thead uses first row as implied header", () => {
		const result = convert("<table><tr><td>Product</td><td>Price</td></tr><tr><td>Apple</td><td>1.00</td></tr></table>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("Product");
    expect(result.content).toContain("Price");
    expect(result.content).toContain("Apple");
    expect(result.content).toContain("1.00");
    expect(result.content).toContain("|");

	}, 30000);

	it("table_pipe_chars_in_content: Table cells containing pipe characters are escaped in output", () => {
		const result = convert("<table><thead><tr><th>Expression</th><th>Result</th></tr></thead><tbody><tr><td>a | b</td><td>true</td></tr></tbody></table>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("Expression");
    expect(result.content).toContain("Result");
    expect(result.content).toContain("true");

	}, 30000);

	it("table_ragged_row_fewer_cells_than_header: A data row with fewer cells than the header is padded to the declared column count", () => {
		const result = convert("<table><tr><th>A</th><th>B</th><th>C</th></tr><tr><td>1</td><td>2</td></tr></table>");
    expect((result.content ?? "")).toBe("| A | B | C |\n| --- | --- | --- |\n| 1 | 2 |   |\n");

	}, 30000);

	it("table_ragged_row_more_cells_than_header: A data row with more cells than the header widens the separator so no cell is silently dropped", () => {
		const result = convert("<table><tr><th>A</th><th>B</th></tr><tr><td>1</td><td>2</td><td>3</td></tr></table>");
    expect((result.content ?? "")).toBe("| A | B |   |\n| --- | --- | --- |\n| 1 | 2 | 3 |\n");

	}, 30000);

	it("table_ragged_rows_padded_not_listed: A headerless table whose rows have different cell counts is padded, not turned into a list", () => {
		const result = convert("<table><tr><td>A</td><td>B</td></tr><tr><td>C</td></tr></table>");
    expect((result.content ?? "")).toBe("| A | B |\n| --- | --- |\n| C |   |\n");

	}, 30000);

	it("table_with_alignment: Table with column alignment attributes", () => {
		const result = convert("<table><thead><tr><th align=\"left\">Left</th><th align=\"center\">Center</th><th align=\"right\">Right</th></tr></thead><tbody><tr><td>L</td><td>C</td><td>R</td></tr></tbody></table>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("Left");
    expect(result.content).toContain("Center");
    expect(result.content).toContain("Right");
    expect(result.content).toContain("L");
    expect(result.content).toContain("C");
    expect(result.content).toContain("R");
    expect(result.content).toContain("|");

	}, 30000);

	it("table_with_colspan: Table with colspan attribute in a header cell", () => {
		const result = convert("<table><thead><tr><th colspan=\"2\">Full Name</th></tr></thead><tbody><tr><td>John</td><td>Doe</td></tr></tbody></table>");
    {
        const _v = result.content;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.content).toContain("Full Name");
    expect(result.content).toContain("John");
    expect(result.content).toContain("Doe");

	}, 30000);

	it("tagged_space_in_strikethrough_kept: A whitespace-only strikethrough keeps the word separator", () => {
		const result = convert("<p>A<del> </del>B</p>");
    expect((result.content ?? "")).toBe("A B\n");

	}, 30000);

	it("tagged_space_in_subscript_kept: A whitespace-only subscript keeps the word separator", () => {
		const result = convert("<p>A<sub> </sub>B</p>");
    expect((result.content ?? "")).toBe("A B\n");

	}, 30000);

	it("tagged_space_not_duplicated: A whitespace-only inline element contributes exactly one space", () => {
		const result = convert("<p>A<i> </i>B</p>");
    expect((result.content ?? "")).toBe("A B\n");

	}, 30000);

	it("unordered_list: Unordered list", () => {
		const result = convert("<ul><li>Item 1</li><li>Item 2</li><li>Item 3</li></ul>");
    expect(result.content).toContain("- Item 1");
    expect(result.content).toContain("- Item 2");
    expect(result.content).toContain("- Item 3");

	}, 30000);

	it("whitespace_only_bold_wrapper_keeps_separator: A bold wrapper holding only a newline-only span still separates the words on either side", () => {
		const result = convert("<b>Alpha</b><b><span>\n</span></b><b>Beta</b>");
    expect((result.content ?? "")).toBe("**Alpha** **Beta**\n");

	}, 30000);

	it("whitespace_only_code_span_kept: A code span whose content is a single space survives", () => {
		const result = convert("<p>A<code> </code>B</p>");
    expect((result.content ?? "")).toBe("A` `B\n");

	}, 30000);

});
