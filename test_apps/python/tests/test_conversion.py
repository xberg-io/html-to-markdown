# This file is auto-generated by alef — DO NOT EDIT.
# alef:hash:e637e29895480a2f0f68cb0f1e4b30406c3049bc27ed1a955c95faaac751886e
# To regenerate: alef generate
# To verify freshness: alef verify

"""E2e tests for category: conversion."""

from html_to_markdown import convert, ConversionOptions, PreprocessingOptions


def test_adjacent_code_spans_merged() -> None:
    """Adjacent code spans merge instead of colliding backticks."""
    html = "<p><code>A</code><code>B</code></p>"

    result = convert(html)
    assert result.content == "`AB`\n"


def test_adjacent_emphasis_merged() -> None:
    """Adjacent emphasis elements share one delimiter run."""
    html = "<p><i>A</i><i>B</i><i>C</i></p>"

    result = convert(html)
    assert result.content == "*ABC*\n"


def test_adjacent_inserted_merged() -> None:
    """Adjacent inserted elements share one delimiter run."""
    html = "<p><ins>A</ins><ins>B</ins></p>"

    result = convert(html)
    assert result.content == "==AB==\n"


def test_adjacent_strikethrough_merged() -> None:
    """Adjacent strikethrough elements share one delimiter run."""
    html = "<p><del>A</del><del>B</del></p>"

    result = convert(html)
    assert result.content == "~~AB~~\n"


def test_adjacent_variable_merged() -> None:
    """Adjacent variable elements share one delimiter run."""
    html = "<p><var>A</var><var>B</var></p>"

    result = convert(html)
    assert result.content == "*AB*\n"


def test_blockquote_code_block_indentation_preserved() -> None:
    """Code block inside a blockquote keeps its line indentation."""
    html = "<blockquote><pre><code>line1\n    line2 indented</code></pre></blockquote>"

    result = convert(html)
    assert result.content == "> ```\n> line1\n>     line2 indented\n> ```\n"


def test_blockquote_multiple_paragraphs() -> None:
    """Blockquote with multiple paragraphs has each paragraph prefixed."""
    html = "<blockquote><p>First paragraph.</p><p>Second paragraph.</p></blockquote>"

    result = convert(html)
    assert result.content is not None
    assert "> First paragraph." in result.content
    assert result.content is not None
    assert "> Second paragraph." in result.content


def test_blockquote_nested() -> None:
    """Nested blockquote produces double-prefixed lines."""
    html = "<blockquote><p>Outer quote.</p><blockquote><p>Inner quote.</p></blockquote></blockquote>"

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "Outer quote." in result.content
    assert result.content is not None
    assert "Inner quote." in result.content


def test_blockquote_nested_list_indentation_preserved() -> None:
    """Nested list inside a blockquote keeps its continuation indentation."""
    html = "<blockquote><ul><li>item a<ul><li>sub a1</li></ul></li></ul></blockquote>"

    result = convert(html)
    assert result.content == "> - item a\n>   * sub a1\n"


def test_blockquote_simple() -> None:
    """Simple blockquote."""
    html = "<blockquote><p>Quote text</p></blockquote>"

    result = convert(html)
    assert result.content is not None
    assert "> Quote text" in result.content


def test_blockquote_text_then_paragraph_gets_blank_line() -> None:
    """Bare text followed by a paragraph inside a blockquote gets a blank-line separator instead of merging into one line."""
    html = "<blockquote>Just text, then <p>a paragraph</p></blockquote>"

    result = convert(html)
    assert result.content == "> Just text, then\n>\n> a paragraph\n"


def test_blockquote_with_list() -> None:
    """Blockquote containing a list preserves list items inside quote."""
    html = "<blockquote><p>Quote intro:</p><ul><li>Point one</li><li>Point two</li></ul></blockquote>"

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "Quote intro:" in result.content
    assert result.content is not None
    assert "Point one" in result.content
    assert result.content is not None
    assert "Point two" in result.content


def test_bold_and_italic() -> None:
    """Nested bold and italic."""
    html = "<p><strong><em>both</em></strong></p>"

    result = convert(html)
    assert result.content is not None
    assert "***both***" in result.content


def test_bold_strong() -> None:
    """Strong tag converts to bold."""
    html = "<p><strong>bold</strong></p>"

    result = convert(html)
    assert result.content is not None
    assert "**bold**" in result.content


def test_br_only_bold_wrapper_keeps_separator() -> None:
    """A bold wrapper holding only a line break still separates the words on either side."""
    html = "<b>Alpha</b><b><br></b><b>Beta</b>"

    result = convert(html)
    assert result.content == "**Alpha** **Beta**\n"


def test_br_only_emphasis_inside_empty_strong_keeps_separator() -> None:
    """A line-break-only emphasis nested inside an otherwise empty strong wrapper still separates the words on either side."""
    html = "<p><strong>Alpha</strong><strong><em><br></em></strong>Beta</p>"

    result = convert(html)
    assert result.content == "**Alpha** Beta\n"


def test_code_block() -> None:
    """Code block with language preserves content."""
    html = "<pre><code class=\"language-python\">print('hello')</code></pre>"

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "print('hello')" in result.content


def test_code_block_br_keeps_real_newline() -> None:
    """A <br> inside a fenced code block stays a real newline, not a split span."""
    html = "<pre><code>A<br>B</code></pre>"

    result = convert(html)
    assert result.content == "```\nA\nB\n```\n"


def test_code_block_no_language() -> None:
    """Code block without a language class preserves content."""
    html = "<pre><code>plain code here</code></pre>"

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "plain code here" in result.content


def test_code_inline_in_paragraph() -> None:
    """Inline code element nested inside a paragraph."""
    html = "<p>Call the <code>initialize()</code> method first.</p>"

    result = convert(html)
    assert result.content is not None
    assert "`initialize()`" in result.content


def test_code_span_br_splits_into_hard_break() -> None:
    """A <br> inside an inline code span splits the span and emits a hard break."""
    html = "<p>X <code>A<br>B<br>C</code></p>"

    result = convert(html)
    assert result.content == "X `A`  \n`B`  \n`C`\n"


def test_code_span_literal_newline_folds_to_space() -> None:
    """A literal line ending inside a code span folds to a space, per CommonMark."""
    html = "<p>see <code>a\nb</code> here</p>"

    result = convert(html)
    assert result.content == "see `a b` here\n"


def test_code_span_suppresses_nested_inline_markers() -> None:
    """Inline markers inside a code span stay literal content."""
    html = "<p><code>a<em>b</em><ins>c</ins></code></p>"

    result = convert(html)
    assert result.content == "`abc`\n"


def test_code_with_backticks_in_content() -> None:
    """Inline code containing backtick characters is properly escaped."""
    html = "<p>Use <code>`backtick` here</code> carefully.</p>"

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "backtick" in result.content


def test_conversion_autolink_filename_not_autolinked() -> None:
    """Bare filename href (no URI scheme) renders as inline link, not autolink (#397)."""
    html = '<a href="foobar.png">foobar.png</a>'

    result = convert(html)
    assert result.content == "[foobar.png](foobar.png)\n"


def test_conversion_autolink_https_url() -> None:
    """Absolute https URL with matching text renders as autolink (#397)."""
    html = '<a href="https://example.com">https://example.com</a>'

    result = convert(html)
    assert result.content == "<https://example.com>\n"


def test_conversion_autolink_mailto() -> None:
    """mailto: link with matching email text renders as autolink (#397)."""
    html = '<a href="mailto:a@b.com">a@b.com</a>'

    result = convert(html)
    assert result.content == "<a@b.com>\n"


def test_conversion_autolink_mixed_filename_and_url() -> None:
    """Mixed filename + URL: only the URL becomes an autolink (#397)."""
    html = '<a href="foobar.png">foobar.png</a> <a href="https://www.heise.de">https://www.heise.de</a>'

    result = convert(html)
    assert result.content == "[foobar.png](foobar.png) <https://www.heise.de>\n"


def test_conversion_autolink_relative_path_not_autolinked() -> None:
    """Relative path href renders as inline link (#397)."""
    html = '<a href="/docs/intro.html">/docs/intro.html</a>'

    result = convert(html)
    assert result.content == "[/docs/intro.html](/docs/intro.html)\n"


def test_div_whitespace_only_span_does_not_indent_image() -> None:
    """A whitespace-only span opening a div does not indent the image after it into a code block."""
    html = '<p>P</p>\n<div>\n<span>    </span><img alt="A" src="S"/>\n</div>'

    result = convert(html)
    assert result.content == "P\n\n![A](S)\n"


def test_emphasis_mark_highlight() -> None:
    """mark tag produces highlighted output."""
    html = "<p><mark>highlighted</mark></p>"

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "highlighted" in result.content


def test_emphasis_strikethrough_del() -> None:
    """del tag converts to GFM strikethrough."""
    html = "<p><del>deleted text</del></p>"

    result = convert(html)
    assert result.content is not None
    assert "~~deleted text~~" in result.content


def test_emphasis_strikethrough_s() -> None:
    """s tag converts to GFM strikethrough."""
    html = "<p><s>strikethrough</s></p>"

    result = convert(html)
    assert result.content is not None
    assert "~~strikethrough~~" in result.content


def test_emphasis_subscript() -> None:
    """sub tag content is preserved."""
    html = "<p>H<sub>2</sub>O</p>"

    result = convert(html)
    assert result.content is not None
    assert "H" in result.content
    assert result.content is not None
    assert "2" in result.content
    assert result.content is not None
    assert "O" in result.content


def test_emphasis_superscript() -> None:
    """sup tag content is preserved."""
    html = "<p>x<sup>2</sup></p>"

    result = convert(html)
    assert result.content is not None
    assert "x" in result.content
    assert result.content is not None
    assert "2" in result.content


def test_emphasis_underline_u() -> None:
    """u tag content is preserved in output."""
    html = "<p><u>underlined</u></p>"

    result = convert(html)
    assert result.content is not None
    assert "underlined" in result.content


def test_form_input_elements() -> None:
    """Form input elements produce readable output without form mechanics."""
    html = '<form><label for="name">Name:</label><input type="text" id="name" placeholder="Enter name"></form>'
    options = ConversionOptions(preprocessing=PreprocessingOptions(remove_forms=False))

    result = convert(html, options)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "Name" in result.content


def test_form_select_options() -> None:
    """Select element with options produces readable output."""
    html = '<form><label>Color:</label><select><option value="red">Red</option><option value="blue" selected>Blue</option><option value="green">Green</option></select></form>'
    options = ConversionOptions(preprocessing=PreprocessingOptions(remove_forms=False))

    result = convert(html, options)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "Color" in result.content


def test_form_textarea() -> None:
    """Textarea element produces readable output."""
    html = "<form><label>Message:</label><textarea>Default text content</textarea></form>"
    options = ConversionOptions(preprocessing=PreprocessingOptions(remove_forms=False))

    result = convert(html, options)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "Message" in result.content


def test_heading_br_only_nested_emphasis_keeps_separator() -> None:
    """A line break inside nested emphasis in a heading still separates the words on either side."""
    html = "<h2><strong>Alpha</strong><strong><em><br></em></strong>Beta</h2>"

    result = convert(html)
    assert result.content == "## **Alpha** Beta\n"


def test_heading_h1() -> None:
    """H1 heading."""
    html = "<h1>Heading 1</h1>"

    result = convert(html)
    assert result.content == "# Heading 1\n"


def test_heading_h2() -> None:
    """H2 heading."""
    html = "<h2>Heading 2</h2>"

    result = convert(html)
    assert result.content == "## Heading 2\n"


def test_heading_h3() -> None:
    """H3 heading."""
    html = "<h3>Heading 3</h3>"

    result = convert(html)
    assert result.content == "### Heading 3\n"


def test_heading_h4() -> None:
    """H4 heading."""
    html = "<h4>Heading 4</h4>"

    result = convert(html)
    assert result.content == "#### Heading 4\n"


def test_heading_h5() -> None:
    """H5 heading."""
    html = "<h5>Heading 5</h5>"

    result = convert(html)
    assert result.content == "##### Heading 5\n"


def test_heading_h6() -> None:
    """H6 heading."""
    html = "<h6>Heading 6</h6>"

    result = convert(html)
    assert result.content == "###### Heading 6\n"


def test_image_alt_unmatched_open_bracket_is_escaped() -> None:
    """An unmatched [ in image alt text is escaped so the image survives."""
    html = '<img alt="[A;B)" src="S">'

    result = convert(html)
    assert result.content == "![\\[A;B)](S)\n"


def test_image_figure_figcaption() -> None:
    """Figure with figcaption preserves both image and caption."""
    html = (
        '<figure><img src="sunset.jpg" alt="A sunset"><figcaption>Beautiful sunset over the ocean</figcaption></figure>'
    )

    result = convert(html)
    assert result.content is not None
    assert "![A sunset](sunset.jpg)" in result.content
    assert result.content is not None
    assert "Beautiful sunset over the ocean" in result.content


def test_image_kept_inline_in_link() -> None:
    """keep_inline_images_in naming 'a' keeps an image inside a link that also holds a block element."""
    html = '<a href="https://example.com/file.pdf"><img src="pdf.png"><p>Download PDF</p></a>'
    options = ConversionOptions(keep_inline_images_in=["a"])

    result = convert(html, options)
    assert result.content is not None
    assert "![](pdf.png)" in result.content
    assert result.content is not None
    assert "Download PDF" in result.content
    assert result.content is not None
    assert "https://example.com/file.pdf" in result.content


def test_image_linked() -> None:
    """Image inside an anchor produces a linked image."""
    html = '<a href="https://example.com"><img src="icon.png" alt="Icon"></a>'

    result = convert(html)
    assert result.content is not None
    assert "![Icon](icon.png)" in result.content
    assert result.content is not None
    assert "https://example.com" in result.content


def test_image_multiline_alt_block_marker_is_escaped() -> None:
    """A block marker on a continuation line of multiline alt text is escaped."""
    html = '<img alt="Alpha\n-\nBeta" src="image.png">'

    result = convert(html)
    assert result.content is not None
    assert "![Alpha\n\\-\nBeta](image.png)" in result.content


def test_image_no_alt() -> None:
    """Image without alt text produces image markdown."""
    html = '<img src="banner.jpg">'

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "banner.jpg" in result.content


def test_image_simple() -> None:
    """Image with alt text."""
    html = '<img src="photo.jpg" alt="A photo">'

    result = convert(html)
    assert result.content is not None
    assert "![A photo](photo.jpg)" in result.content


def test_image_src_entity_shaped_ampersand_kept_literal() -> None:
    """An & that would read as a character reference in an image destination stays literal."""
    html = '<img src="?&amp;plus;">'

    result = convert(html)
    assert result.content == "![](?&amp;plus;)\n"


def test_image_with_title() -> None:
    """Image with title attribute includes title in output."""
    html = '<img src="chart.png" alt="Sales chart" title="Q3 Sales">'

    result = convert(html)
    assert result.content is not None
    assert "![Sales chart](chart.png" in result.content
    assert result.content is not None
    assert "Q3 Sales" in result.content


def test_inline_code() -> None:
    """Inline code."""
    html = "<p>Use <code>console.log()</code> to debug</p>"

    result = convert(html)
    assert result.content is not None
    assert "`console.log()`" in result.content


def test_italic_em() -> None:
    """Em tag converts to italic."""
    html = "<p><em>italic</em></p>"

    result = convert(html)
    assert result.content is not None
    assert "*italic*" in result.content


def test_line_break_br_tag() -> None:
    """Single br tag produces a line break in output."""
    html = "<p>First line.<br>Second line.</p>"

    result = convert(html)
    assert result.content is not None
    assert "First line." in result.content
    assert result.content is not None
    assert "Second line." in result.content


def test_line_break_hr_tag() -> None:
    """hr tag produces a horizontal separator between content."""
    html = "<p>Before rule.</p><hr><p>After rule.</p>"

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "Before rule." in result.content
    assert result.content is not None
    assert "After rule." in result.content


def test_line_break_multiple_br() -> None:
    """Multiple consecutive br tags in sequence."""
    html = "<p>Start.<br><br>End.</p>"

    result = convert(html)
    assert result.content is not None
    assert "Start." in result.content
    assert result.content is not None
    assert "End." in result.content


def test_link_anchor_fragment() -> None:
    """Fragment-only anchor link is preserved."""
    html = '<a href="#section">Jump to section</a>'

    result = convert(html)
    assert result.content is not None
    assert "[Jump to section](#section)" in result.content


def test_link_empty_href() -> None:
    """Link with empty href produces output with the link text."""
    html = '<a href="">No destination</a>'

    result = convert(html)
    assert result.content is not None
    assert "No destination" in result.content


def test_link_href_entity_shaped_ampersand_kept_literal() -> None:
    """An & that would read as a character reference in a link destination or title stays literal."""
    html = '<a href="?&amp;plus;" title="&amp;plus;">T</a>'

    result = convert(html)
    assert result.content == '[T](?&amp;plus; "&amp;plus;")\n'


def test_link_image_inside() -> None:
    """Image inside a link produces a linked image."""
    html = '<a href="https://example.com"><img src="logo.png" alt="Logo"></a>'

    result = convert(html)
    assert result.content is not None
    assert "![Logo](logo.png)" in result.content
    assert result.content is not None
    assert "https://example.com" in result.content


def test_link_label_block_marker_is_escaped() -> None:
    """A block marker on a continuation line of a link label is escaped."""
    html = '<a href="https://example.com">Alpha<br>-<br>Beta</a>'

    result = convert(html)
    assert result.content is not None
    assert "[Alpha  \n\\-  \nBeta](https://example.com)" in result.content


def test_link_label_unmatched_open_bracket_is_escaped() -> None:
    """An unmatched [ in a link label is escaped so the link survives."""
    html = '<a href="S">[A;B)</a>'

    result = convert(html)
    assert result.content == "[\\[A;B)](S)\n"


def test_link_leading_line_break_is_kept() -> None:
    """A <br> just after <a> is kept as a hard line break in the label."""
    html = 'Beta<a href="https://example.com"><br>Alpha</a>'

    result = convert(html)
    assert result.content is not None
    assert "Beta[  \nAlpha](https://example.com)" in result.content


def test_link_mailto() -> None:
    """Mailto link is preserved with mailto: scheme."""
    html = '<a href="mailto:user@example.com">Email us</a>'

    result = convert(html)
    assert result.content is not None
    assert "mailto:user@example.com" in result.content


def test_link_simple() -> None:
    """Simple link."""
    html = '<a href="https://example.com">Example</a>'

    result = convert(html)
    assert result.content is not None
    assert "[Example](https://example.com)" in result.content


def test_link_split_by_block_child_is_not_duplicated() -> None:
    """An anchor wrapping a block that holds another anchor records its destination once, not twice."""
    html = '<a href="/o"><div><a href="/i">Inner</a></div></a>'

    result = convert(html)
    assert result.content == "[](/o)\n\n[Inner](/i)\n"


def test_link_split_drops_empty_authored_half() -> None:
    """An anchor split around a block drops its empty half when the reconstructed half carries the text."""
    html = '<a href="/o"><div>Text<a href="/i">Inner</a></div></a>'

    result = convert(html)
    assert result.content == "[Text](/o)[Inner](/i)\n"


def test_link_split_keeps_text_bearing_half() -> None:
    """An anchor split around a block keeps the half that carries its text."""
    html = '<a href="/o">Text<div><a href="/i">Inner</a></div></a>'

    result = convert(html)
    assert result.content == "[Text](/o)\n\n[Inner](/i)\n"


def test_link_trailing_line_break_is_kept() -> None:
    """A <br> just before </a> is kept as a hard line break in the label."""
    html = '<a href="https://example.com">Alpha<br></a>Beta'

    result = convert(html)
    assert result.content is not None
    assert "[Alpha  \n](https://example.com)Beta" in result.content


def test_link_with_bold_text() -> None:
    """Link containing bold text preserves formatting."""
    html = '<a href="https://example.com"><strong>Bold link</strong></a>'

    result = convert(html)
    assert result.content is not None
    assert "**Bold link**" in result.content
    assert result.content is not None
    assert "https://example.com" in result.content


def test_link_with_title() -> None:
    """Link with title attribute."""
    html = '<a href="https://example.com" title="Example Site">Example</a>'

    result = convert(html)
    assert result.content is not None
    assert "[Example](https://example.com" in result.content
    assert result.content is not None
    assert "Example Site" in result.content


def test_link_wrapping_table_renders_table() -> None:
    """An anchor wrapping a table renders the table as a block instead of an escaped link label."""
    html = '<a href="https://example.com/outer"><table><tr><td><a href="https://example.com/inner">Inner</a></td></tr></table></a>'

    result = convert(html)
    assert result.content is not None
    assert "[https://example.com/outer](https://example.com/outer)" in result.content
    assert result.content is not None
    assert "[Inner](https://example.com/inner)" in result.content
    assert result.content is None or "\\|" not in result.content


def test_list_definition_dl() -> None:
    """Definition list with dt and dd elements."""
    html = "<dl><dt>Term One</dt><dd>Definition of term one.</dd><dt>Term Two</dt><dd>Definition of term two.</dd></dl>"

    result = convert(html)
    assert result.content is not None
    assert "Term One" in result.content
    assert result.content is not None
    assert "Definition of term one." in result.content
    assert result.content is not None
    assert "Term Two" in result.content
    assert result.content is not None
    assert "Definition of term two." in result.content


def test_list_item_multiple_paragraphs() -> None:
    """List item containing multiple paragraphs."""
    html = "<ul><li><p>First paragraph in item.</p><p>Second paragraph in item.</p></li><li>Simple item</li></ul>"

    result = convert(html)
    assert result.content is not None
    assert "First paragraph in item." in result.content
    assert result.content is not None
    assert "Second paragraph in item." in result.content
    assert result.content is not None
    assert "Simple item" in result.content


def test_list_mixed_nested() -> None:
    """Mixed list: ordered list nested inside unordered list."""
    html = "<ul><li>Item A<ol><li>Sub 1</li><li>Sub 2</li></ol></li><li>Item B</li></ul>"

    result = convert(html)
    assert result.content is not None
    assert "Item A" in result.content
    assert result.content is not None
    assert "Sub 1" in result.content
    assert result.content is not None
    assert "Sub 2" in result.content
    assert result.content is not None
    assert "Item B" in result.content


def test_list_nested_ordered() -> None:
    """Nested ordered list with two levels of depth."""
    html = "<ol><li>Step 1<ol><li>Step 1a</li><li>Step 1b</li></ol></li><li>Step 2</li></ol>"

    result = convert(html)
    assert result.content is not None
    assert "Step 1" in result.content
    assert result.content is not None
    assert "Step 1a" in result.content
    assert result.content is not None
    assert "Step 1b" in result.content
    assert result.content is not None
    assert "Step 2" in result.content


def test_list_nested_unordered() -> None:
    """Nested unordered list with two levels of depth."""
    html = "<ul><li>Parent A<ul><li>Child A1</li><li>Child A2</li></ul></li><li>Parent B</li></ul>"

    result = convert(html)
    assert result.content is not None
    assert "Parent A" in result.content
    assert result.content is not None
    assert "Child A1" in result.content
    assert result.content is not None
    assert "Child A2" in result.content
    assert result.content is not None
    assert "Parent B" in result.content


def test_list_task_checkboxes() -> None:
    """Task list with checked and unchecked checkboxes."""
    html = '<ul><li><input type="checkbox" checked> Done task</li><li><input type="checkbox"> Pending task</li></ul>'

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "Done task" in result.content
    assert result.content is not None
    assert "Pending task" in result.content


def test_nested_newline_only_spans_after_emphasis_keep_separator() -> None:
    """A newline inside two nested spans after an emphasis still separates the words on either side."""
    html = "<p><i>Alpha</i><span><span>\n</span></span>Beta</p>"

    result = convert(html)
    assert result.content == "*Alpha* Beta\n"


def test_nested_newline_only_spans_keep_separator() -> None:
    """A newline inside two nested spans still separates the words on either side."""
    html = "<p>Alpha<span><span>\n</span></span>Beta</p>"

    result = convert(html)
    assert result.content == "Alpha Beta\n"


def test_ordered_list() -> None:
    """Ordered list."""
    html = "<ol><li>First</li><li>Second</li><li>Third</li></ol>"

    result = convert(html)
    assert result.content is not None
    assert "1. First" in result.content
    assert result.content is not None
    assert "2. Second" in result.content
    assert result.content is not None
    assert "3. Third" in result.content


def test_paragraph_multiple() -> None:
    """Multiple paragraphs are separated by a blank line."""
    html = "<p>First paragraph.</p><p>Second paragraph.</p>"

    result = convert(html)
    assert result.content is not None
    assert "First paragraph." in result.content
    assert result.content is not None
    assert "Second paragraph." in result.content


def test_paragraph_nested_divs() -> None:
    """Text nested inside divs is extracted correctly."""
    html = "<div><div><p>Nested text</p></div></div>"

    result = convert(html)
    assert result.content is not None
    assert "Nested text" in result.content


def test_paragraph_simple() -> None:
    """Simple paragraph converts to plain text."""
    html = "<p>Hello World</p>"

    result = convert(html)
    assert result.content == "Hello World\n"


def test_paragraph_whitespace_only_span_separates_words() -> None:
    """A newline-only inline wrapper still separates the words on either side."""
    html = 'Alpha<span style="white-space:pre">\n</span>13'

    result = convert(html)
    assert result.content == "Alpha 13\n"


def test_paragraph_with_inline_formatting() -> None:
    """Paragraph with bold, italic, and a link."""
    html = '<p>This has <strong>bold</strong>, <em>italic</em>, and a <a href="https://example.com">link</a>.</p>'

    result = convert(html)
    assert result.content is not None
    assert "**bold**" in result.content
    assert result.content is not None
    assert "*italic*" in result.content
    assert result.content is not None
    assert "[link](https://example.com)" in result.content


def test_paragraph_with_line_breaks() -> None:
    """Paragraph with br tags produces line breaks in output."""
    html = "<p>Line one.<br>Line two.<br>Line three.</p>"

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "Line one." in result.content
    assert result.content is not None
    assert "Line two." in result.content
    assert result.content is not None
    assert "Line three." in result.content


def test_semantic_abbr() -> None:
    """Abbreviation element text is preserved."""
    html = '<p>The <abbr title="World Wide Web">WWW</abbr> is global.</p>'

    result = convert(html)
    assert result.content is not None
    assert "WWW" in result.content


def test_semantic_article() -> None:
    """Article element wrapping content preserves inner content."""
    html = "<article><h2>Article Title</h2><p>Article body.</p></article>"

    result = convert(html)
    assert result.content is not None
    assert "Article Title" in result.content
    assert result.content is not None
    assert "Article body." in result.content


def test_semantic_definition_list() -> None:
    """Definition list with term and description."""
    html = "<dl><dt>HTML</dt><dd>HyperText Markup Language</dd><dt>CSS</dt><dd>Cascading Style Sheets</dd></dl>"

    result = convert(html)
    assert result.content is not None
    assert "HTML" in result.content
    assert result.content is not None
    assert "HyperText Markup Language" in result.content
    assert result.content is not None
    assert "CSS" in result.content
    assert result.content is not None
    assert "Cascading Style Sheets" in result.content


def test_semantic_details_summary() -> None:
    """Details and summary elements produce readable output."""
    html = "<details><summary>Click to expand</summary><p>Hidden content here.</p></details>"

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "Click to expand" in result.content


def test_semantic_hr() -> None:
    """Horizontal rule produces a separator in output."""
    html = "<p>Above</p><hr><p>Below</p>"

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "Above" in result.content
    assert result.content is not None
    assert "Below" in result.content


def test_semantic_mark_highlight() -> None:
    """Mark tag produces highlighted output."""
    html = "<p>This is <mark>highlighted text</mark> in a sentence.</p>"

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "highlighted text" in result.content


def test_semantic_section_with_heading() -> None:
    """Section element with heading preserves structure."""
    html = "<section><h3>Section Heading</h3><p>Section content.</p></section>"

    result = convert(html)
    assert result.content is not None
    assert "Section Heading" in result.content
    assert result.content is not None
    assert "Section content." in result.content


def test_semantic_sub_superscript() -> None:
    """Subscript and superscript elements are preserved in output."""
    html = "<p>H<sub>2</sub>O and E=mc<sup>2</sup></p>"

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "H" in result.content
    assert result.content is not None
    assert "2" in result.content
    assert result.content is not None
    assert "O" in result.content
    assert result.content is not None
    assert "E=mc" in result.content


def test_simple_table() -> None:
    """Simple table with header."""
    html = "<table><thead><tr><th>Name</th><th>Age</th></tr></thead><tbody><tr><td>Alice</td><td>30</td></tr></tbody></table>"

    result = convert(html)
    assert result.content is not None
    assert "Name" in result.content
    assert result.content is not None
    assert "Age" in result.content
    assert result.content is not None
    assert "Alice" in result.content
    assert result.content is not None
    assert "30" in result.content
    assert result.content is not None
    assert "|" in result.content
    assert result.content is not None
    assert "---" in result.content


def test_space_only_italic_inside_empty_strong_keeps_separator() -> None:
    """A space-only italic nested inside an otherwise empty strong wrapper still separates the words on either side."""
    html = "<p><strong>Alpha</strong><strong><i> </i></strong>Beta</p>"

    result = convert(html)
    assert result.content == "**Alpha** Beta\n"


def test_strike_renders_as_strikethrough() -> None:
    """The deprecated strike tag renders as strikethrough."""
    html = "<p><strike>gone</strike></p>"

    result = convert(html)
    assert result.content == "~~gone~~\n"


def test_table_cell_br_only_emphasis_keeps_separator() -> None:
    """A line-break-only emphasis inside a table cell still separates the words on either side."""
    html = "<table><tr><td><strong>Alpha</strong><em><br></em>Beta</td></tr></table>"

    result = convert(html)
    assert result.content == "| **Alpha** Beta |\n| -------------- |\n"


def test_table_content_after_unclosed_row_survives() -> None:
    """Content following a table whose last row is never closed is still converted."""
    html = "<table><tr><th>Before</th></tr><tr></table><p>Visible footer</p>"

    result = convert(html)
    assert result.content is not None
    assert "| Before |" in result.content
    assert result.content is not None
    assert "| ------ |" in result.content
    assert result.content is not None
    assert "Visible footer" in result.content


def test_table_empty() -> None:
    """Empty table produces no output or minimal output."""
    html = "<table></table>"

    result = convert(html)
    assert result.content == ""


def test_table_layout_cell_paragraphs_stay_separated() -> None:
    """Adjacent paragraphs in a layout-table cell are separated rather than concatenated into one run of text."""
    html = "<table><tr><td><p><b>Alice Example</b></p><p><i>Customer Service</i></p></td><td>Logo</td></tr><tr><td>A</td><td>B</td><td>C</td></tr></table>"
    options = ConversionOptions(compact_tables=True)

    result = convert(html, options)
    assert result.content is None or "**Alice Example***Customer Service*" not in result.content
    assert result.content is not None
    assert "Alice Example" in result.content
    assert result.content is not None
    assert "Customer Service" in result.content
    assert result.content is not None
    assert "Logo" in result.content


def test_table_layout_row_keeps_image() -> None:
    """A layout-table row keeps an image as Markdown instead of degrading it to alt text."""
    html = '<table><tr><td><img src="a.svg" alt="A"> <a href="/1">x</a></td><td><a href="/2">y</a></td></tr><tr><td><a href="/3">z</a></td></tr></table>'

    result = convert(html)
    assert result.content == "- ![A](a.svg) [x](/1) [y](/2)\n- [z](/3)\n"


def test_table_nested_chain_not_misclassified_as_layout() -> None:
    """A straight chain of one-nested-table-per-cell tables renders as pipe tables, not a broken layout list."""
    html = "<table><tr><td><table><tr><td><table><tr><td>leaf</td></tr></table></td></tr></table></td></tr></table>"

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "leaf" in result.content
    assert result.content is not None
    assert "| ---" in result.content


def test_table_nested_in_single_cell_renders_separately() -> None:
    """A table nested in a single-cell row renders as its own table."""
    html = "<table><tr><th>Summary</th></tr><tr><td>Before<table><tr><th>Item</th><th>Amount</th></tr><tr><td>Alpha</td><td>10</td></tr></table></td></tr></table>"

    result = convert(html)
    assert result.content is not None
    assert "| Item  | Amount |" in result.content
    assert result.content is not None
    assert "| Alpha | 10     |" in result.content
    assert result.content is None or "\\|" not in result.content


def test_table_nested_in_wrapper_renders_separately() -> None:
    """A nested table wrapped in a div still renders as its own table rather than raw pipes."""
    html = "<table><tr><th>Summary</th></tr><tr><td>Before<div><table><tr><th>Item</th><th>Amount</th></tr><tr><td>Alpha</td><td>10</td></tr></table></div></td></tr></table>"

    result = convert(html)
    assert result.content is not None
    assert "| Item  | Amount |" in result.content
    assert result.content is not None
    assert "| Alpha | 10     |" in result.content
    assert result.content is not None
    assert "Before" in result.content
    assert result.content is None or "\\|" not in result.content


def test_table_no_thead() -> None:
    """Table without thead uses first row as implied header."""
    html = "<table><tr><td>Product</td><td>Price</td></tr><tr><td>Apple</td><td>1.00</td></tr></table>"

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "Product" in result.content
    assert result.content is not None
    assert "Price" in result.content
    assert result.content is not None
    assert "Apple" in result.content
    assert result.content is not None
    assert "1.00" in result.content
    assert result.content is not None
    assert "|" in result.content


def test_table_pipe_chars_in_content() -> None:
    """Table cells containing pipe characters are escaped in output."""
    html = "<table><thead><tr><th>Expression</th><th>Result</th></tr></thead><tbody><tr><td>a | b</td><td>true</td></tr></tbody></table>"

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "Expression" in result.content
    assert result.content is not None
    assert "Result" in result.content
    assert result.content is not None
    assert "true" in result.content


def test_table_ragged_row_fewer_cells_than_header() -> None:
    """A data row with fewer cells than the header is padded to the declared column count."""
    html = "<table><tr><th>A</th><th>B</th><th>C</th></tr><tr><td>1</td><td>2</td></tr></table>"

    result = convert(html)
    assert result.content == "| A | B | C |\n| --- | --- | --- |\n| 1 | 2 |   |\n"


def test_table_ragged_row_more_cells_than_header() -> None:
    """A data row with more cells than the header widens the separator so no cell is silently dropped."""
    html = "<table><tr><th>A</th><th>B</th></tr><tr><td>1</td><td>2</td><td>3</td></tr></table>"

    result = convert(html)
    assert result.content == "| A | B |   |\n| --- | --- | --- |\n| 1 | 2 | 3 |\n"


def test_table_ragged_rows_padded_not_listed() -> None:
    """A headerless table whose rows have different cell counts is padded, not turned into a list."""
    html = "<table><tr><td>A</td><td>B</td></tr><tr><td>C</td></tr></table>"

    result = convert(html)
    assert result.content == "| A | B |\n| --- | --- |\n| C |   |\n"


def test_table_with_alignment() -> None:
    """Table with column alignment attributes."""
    html = '<table><thead><tr><th align="left">Left</th><th align="center">Center</th><th align="right">Right</th></tr></thead><tbody><tr><td>L</td><td>C</td><td>R</td></tr></tbody></table>'

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "Left" in result.content
    assert result.content is not None
    assert "Center" in result.content
    assert result.content is not None
    assert "Right" in result.content
    assert result.content is not None
    assert "L" in result.content
    assert result.content is not None
    assert "C" in result.content
    assert result.content is not None
    assert "R" in result.content
    assert result.content is not None
    assert "|" in result.content


def test_table_with_colspan() -> None:
    """Table with colspan attribute in a header cell."""
    html = '<table><thead><tr><th colspan="2">Full Name</th></tr></thead><tbody><tr><td>John</td><td>Doe</td></tr></tbody></table>'

    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "Full Name" in result.content
    assert result.content is not None
    assert "John" in result.content
    assert result.content is not None
    assert "Doe" in result.content


def test_tagged_space_in_strikethrough_kept() -> None:
    """A whitespace-only strikethrough keeps the word separator."""
    html = "<p>A<del> </del>B</p>"

    result = convert(html)
    assert result.content == "A B\n"


def test_tagged_space_in_subscript_kept() -> None:
    """A whitespace-only subscript keeps the word separator."""
    html = "<p>A<sub> </sub>B</p>"

    result = convert(html)
    assert result.content == "A B\n"


def test_tagged_space_not_duplicated() -> None:
    """A whitespace-only inline element contributes exactly one space."""
    html = "<p>A<i> </i>B</p>"

    result = convert(html)
    assert result.content == "A B\n"


def test_unordered_list() -> None:
    """Unordered list."""
    html = "<ul><li>Item 1</li><li>Item 2</li><li>Item 3</li></ul>"

    result = convert(html)
    assert result.content is not None
    assert "- Item 1" in result.content
    assert result.content is not None
    assert "- Item 2" in result.content
    assert result.content is not None
    assert "- Item 3" in result.content


def test_whitespace_only_bold_wrapper_keeps_separator() -> None:
    """A bold wrapper holding only a newline-only span still separates the words on either side."""
    html = "<b>Alpha</b><b><span>\n</span></b><b>Beta</b>"

    result = convert(html)
    assert result.content == "**Alpha** **Beta**\n"


def test_whitespace_only_code_span_kept() -> None:
    """A code span whose content is a single space survives."""
    html = "<p>A<code> </code>B</p>"

    result = convert(html)
    assert result.content == "A` `B\n"
