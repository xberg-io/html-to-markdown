...
// alef:hash:20cf05495000440d666df0804588539ea995b0a32e14b0bd0a9da5b8d50613c4
...
$repoRoot = __DIR__ . '/../../';
$localExtCandidates = [
    $repoRoot . 'target/release/libhtml_to_markdown_php' . $extSuffix,
    $repoRoot . 'crates/html-to-markdown-php/target/release/libhtml_to_markdown_php' . $extSuffix,
];
...
$resolvePieFromExtensionDir = true;
...
if (!$pieInstalledExtPath && $resolvePieFromExtensionDir) {
    ...
    if ($registryExtDir !== '') {
        $pieInstalledExtPath = $registryExtDir . '/html_to_markdown.so';
    }
}
...
if (!file_exists($extPath)) {
    fwrite(STDERR, "error: no html_to_markdown PHP extension build found.\n");
    foreach ($localExtCandidates as $candidate) {
    ...
    fwrite(STDERR, "  cargo build --release -p html-to-markdown-php\n");
    exit(1);
}
...
$php = PHP_BINARY;
$isolatedIni = alef_build_isolated_ini($php, 'html_to_markdown');
...
$preflightCmd = array_merge([$php], $phpConfigArgs, ['-r', $preflightScript]);
...
$process = proc_open(
    ...
    $pipes,
    null,
    ['ALEF_PREFLIGHT_EXT_NAME' => 'html_to_markdown'] + $_ENV,
    ['bypass_shell' => true],
);
if ($process === false) {
    fwrite(STDERR, "error: failed to spawn PHP preflight check for the html_to_markdown extension.\n");
    exit(1);
}
...
if (stripos($preflightStderr, 'already loaded') !== false) {
    fwrite(STDERR, "error: the html_to_markdown extension is already loaded from an ambient php.ini\n");
...
if ($preflightExit !== 0 || $preflightStdout === '') {
    fwrite(STDERR, "error: the html_to_markdown extension did not load from $extPath.\n");
...
if (version_compare($loadedVersion, '3.15.0', '<')) {
    fwrite(STDERR, "error: loaded html_to_markdown extension version mismatch.\n");
    fwrite(STDERR, "  extension at $extPath reports version: $loadedVersion\n");
    fwrite(STDERR, "  expected version: >=3.15.0\n");
...
$cmd = array_merge([$php], $phpConfigArgs, [$phpunitPath], array_slice($GLOBALS['argv'], 1));
