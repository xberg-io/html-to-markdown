...
// alef:hash:d8bf29ef4f52f341d95990ece1a60b4813888a54b490073ebc2926f5cdd2698a
// alef:handle-abi:1
...
/// Must run before any deferred `_free` call: every `htm_*` FFI entry point
...
pub const ConversionOptions = struct {
    ...
    ///
    /// Default `null` — no resolution, output byte-identical to versions before this
    /// option existed.
    base_url: ?[]const u8,
...
pub fn convert(html: []const u8, options: ?[]const u8) ConversionError![]u8 {
    ...
    const options_handle = if (options_z) |z| c.htm_conversion_options_from_json(z) else 0;
    if (options_z != null and options_handle == 0) return _error_with_message(ConversionError);
    defer if (options_handle != 0) c.htm_conversion_options_free(options_handle);
    ...
    return blk: {
        ...
        const value = _result;
        defer c.htm_conversion_result_free(value);
        const _json_ptr = c.htm_conversion_result_to_json(value);
...
/// every other opaque-handle constructor in this module.
pub fn html_visitor_handle_from_callbacks(callbacks: c.HTMHtmVisitorCallbacks) VisitorHandle {
    var _cb = callbacks;
    return c.htm_visitor_create(&_cb);
}
