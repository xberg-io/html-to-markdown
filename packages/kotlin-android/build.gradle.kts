...
// alef:hash:34c81f8dd9555d1ba8c3958df2563b19ade9f2e2f0d9ea7c25bd38e3f5bcda77
...
tasks.register("buildHostJni", Exec::class) {
    if (project.providers.gradleProperty("alef.skipHostJni").orNull != "true") {
        ...
        val jniManifest = configuredManifest?.let(::file) ?: generateSequence(projectDir) { it.parentFile }
            .map { it.resolve("crates/html-to-markdown-rs-jni/Cargo.toml") }
            .firstOrNull { it.isFile }
            ?: throw GradleException(
                "Cannot locate crates/html-to-markdown-rs-jni/Cargo.toml from $projectDir; " +
...
tasks.register("copyHostJni", Copy::class) {
    if (project.providers.gradleProperty("alef.skipHostJni").orNull != "true") {
        ...
        val jniManifest = configuredManifest?.let(::file) ?: generateSequence(projectDir) { it.parentFile }
            .map { it.resolve("crates/html-to-markdown-rs-jni/Cargo.toml") }
            .firstOrNull { it.isFile }
            ?: throw GradleException("Cannot locate crates/html-to-markdown-rs-jni/Cargo.toml from $projectDir")
...
tasks.register("validateJniLibsForRelease") {
    doFirst {
        ...
        if (releaseAssemble) {
            ...
            // lib*_ffi.so instead of libhtm_jni.so) passes the emptiness
            ...
            if (abisMissingJniLib.isNotEmpty()) {
                throw GradleException(
                    ...
                    "export the JNI symbols System.loadLibrary(\"htm_jni\") requires and " +
...
tasks.register("buildAndroidJniLibs", Exec::class) {
    val androidAbis = listOf("arm64-v8a", "x86_64", "armeabi-v7a", "x86")
