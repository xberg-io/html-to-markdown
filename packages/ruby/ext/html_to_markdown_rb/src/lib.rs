...
// alef:hash:adcf6160cdf7035806744fc3efe8b3116ce330a5c164ade234e6c2fe49fbe748
...
impl ConversionOptions {
    fn new(args: &[magnus::Value]) -> Result<Self, magnus::Error> {
        ...
        Ok(Self {
            ...
            base_url: match kwargs.get(ruby.to_symbol("base_url")).filter(|v| !v.is_nil()) {
                Some(v) => Some(String::try_convert(v).map_err(|e| {
                    magnus::Error::new(
                        unsafe { magnus::Ruby::get_unchecked() }.exception_type_error(),
                        format!("invalid value for `base_url`: {}", e),
                    )
                })?),
                None => None,
            },
...
impl From<VisitResult> for html_to_markdown_rs::VisitResult {
    fn from(val: VisitResult) -> Self {
        match val {
            VisitResult::Continue => Self::Continue,
            VisitResult::Custom(_0) => Self::Custom(_0),
            VisitResult::Skip => Self::Skip,
            VisitResult::PreserveHtml => Self::PreserveHtml,
            VisitResult::Error(_0) => Self::Error(_0),
        }
    }
}
...
fn ruby_init(ruby: &Ruby) -> Result<(), Error> {
    ...
    class.define_method("base_url", method!(ConversionOptions::base_url, 0))?;
    ...
    class.define_method("base_url", method!(ConversionOptionsUpdate::base_url, 0))?;
