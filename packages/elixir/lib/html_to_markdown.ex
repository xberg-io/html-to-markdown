...
# alef:hash:c07ffe2612cf83ca58b7ce4334e7391488b648ad6cb0fa49a488da53ef00c138
...
defmodule HtmlToMarkdown do
  ...
  defp apply_visitor_callback(fun, args_json) do
    ...
    case result do
      ...
      {:custom, value} -> substitute_template(to_string(value), args)
  ...
  @doc false
  defp substitute_template(template, args) do
    Regex.replace(~r/\{(\w+)\}/, template, fn _, key ->
      case Map.get(args, "_#{key}") do
        nil -> "{#{key}}"
        val -> to_string(val)
      end
    end)
