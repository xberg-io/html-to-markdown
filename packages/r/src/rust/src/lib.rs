...
// alef:hash:b70986d22491b357b6017724d928f0169ba0aedf7c85c12cfd38e8103d96c4a0
...
pub fn new_conversionoptions(
    ...
    if let Some(v) = heading_style
        && let Ok(parsed) = serde_json::from_str(&format!("\"{v}\""))
    {
...
pub fn convert(html: String, options: Robj) -> Result<Robj> {
    ...
    let opts_local = crate::options::decode_options(options).map_err(extendr_api::Error::Other)?;
...
fn nodecontext_to_robj(ctx: &html_to_markdown_rs::NodeContext) -> extendr_api::Robj {
    use extendr_api::prelude::*;
    let pairs: Vec<(&str, extendr_api::Robj)> = vec![
        (
            "node_type",
            extendr_api::Robj::from(format!("{:?}", ctx.node_type).as_str()),
        ),
        ("tag_name", extendr_api::Robj::from(AsRef::<str>::as_ref(&ctx.tag_name))),
        ("depth", extendr_api::Robj::from(ctx.depth as f64)),
        ("index_in_parent", extendr_api::Robj::from(ctx.index_in_parent as f64)),
        (
            "parent_tag",
        ...
        ("is_inline", extendr_api::Robj::from(ctx.is_inline)),
    ];
...
impl html_to_markdown_rs::visitor::HtmlVisitor for RHtmlVisitorBridge {
    ...
    fn visit_element_start(&mut self, _ctx: &html_to_markdown_rs::NodeContext) -> html_to_markdown_rs::VisitResult {
        ...
        let args = extendr_api::Pairlist::from_pairs(&[("ctx", nodecontext_to_robj(_ctx))]);
        let result = fn_robj.call(args);
