...
// alef:hash:ef3507915be3364eda4ed081c78e40e10d47ea13420be2f075920d942c817c36
...
pub fn decode_options(options: Robj) -> std::result::Result<crate::ConversionOptions, String> {
    ...
    if let Some(v) = list_get(&list, "max_depth").filter(|v| !v.is_null()) {
    ...
    if let Some(v) = list_get(&list, "base_url") {
        opts.base_url = Some(String::try_from(&v).map_err(|e| format!("base_url: {e}"))?);
    }
