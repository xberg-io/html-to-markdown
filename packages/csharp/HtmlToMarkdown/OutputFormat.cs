// This file is auto-generated by alef. DO NOT EDIT.
// alef:hash:0104792447b2b220f01fdfd17da1ce7b2d6aec2cec71afadbb9ce4d4564ec24d
#nullable enable

using System;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace HtmlToMarkdown;
/// <summary>
/// Output format for conversion.
///
/// Specifies the target markup language format for the conversion output.
/// </summary>
[JsonConverter(typeof(OutputFormatJsonConverter))]
public enum OutputFormat
{    /// <summary>
    /// Standard Markdown (CommonMark compatible). Default.
    /// </summary>
    [JsonPropertyName("markdown")]
    Markdown,    /// <summary>
    /// Djot lightweight markup language.
    /// </summary>
    [JsonPropertyName("djot")]
    Djot,    /// <summary>
    /// Plain text output (no markup, visible text only).
    /// </summary>
    [JsonPropertyName("plain")]
    Plain,}


/// <summary>Custom JSON converter for <see cref="OutputFormat"/> that respects explicit variant names.</summary>
internal sealed class OutputFormatJsonConverter : JsonConverter<OutputFormat>
{
    public override OutputFormat Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        var value = reader.GetString();
        return value switch
        {            "markdown" => OutputFormat.Markdown,            "djot" => OutputFormat.Djot,            "plain" => OutputFormat.Plain,            _ => throw new JsonException($"Unknown OutputFormat value: {value}")
        };
    }

    public override void Write(Utf8JsonWriter writer, OutputFormat value, JsonSerializerOptions options)
    {
        var str = value switch
        {            OutputFormat.Markdown => "markdown",            OutputFormat.Djot => "djot",            OutputFormat.Plain => "plain",            _ => throw new JsonException($"Unknown OutputFormat value: {value}")
        };
        writer.WriteStringValue(str);
    }
}
