...
# alef:hash:732f9483342c5cab04532462dcce28824d178d3ad0916039361af434cbd0c4d4
...
test_that("issue_396_backticks_blank_line_after_fence: Backticks code block trims trailing newline inside fence and adds blank line after closing fence (issue #396)", {
  ...
  expect_equal(result$content, "Foo\n\n```\n1\n2\n```\n\nBar\n")
})
...
test_that("options_br_in_tables_anchor_wrapped_nested_table_in_layout_cell: An anchor-wrapped table inside a layout cell keeps its inner links clickable", {
  ...
  expect_equal(result$content, "- [/outer](/outer)\n\n| [One](/one)[Two](/two) |\n| --- |\n- After\n")
})
...
test_that("options_br_in_tables_nested_data_table: A single-cell layout wrapper preserves its nested data table as structured Markdown", {
  ...
  expect_equal(result$content, "Before\n\n| ID | Status |\n| --- | --- |\n| 123 | Done |\n")
})

test_that("options_br_in_tables_nested_table_rows: A nested table flattened into a cell keeps its row boundaries as literal <br> when br_in_tables is enabled", {
  ...
  expect_true(any(grepl("Before<br>", result$content, fixed = TRUE)))
  expect_true(any(grepl("<br>", result$content, fixed = TRUE)))
  expect_true(any(grepl("123", result$content, fixed = TRUE)))
  expect_true(any(grepl("Done", result$content, fixed = TRUE)))
  expect_true(any(grepl("Status", result$content, fixed = TRUE)))
})
...
test_that("options_custom_bullets_apply_to_layout_table_rows: A layout table renders its rows as list items, and those markers honour the bullets option like any other list", {
  ...
  expect_true(grepl("* alpha", result$content, fixed = TRUE))
  expect_false(grepl("- alpha", result$content, fixed = TRUE))
})
...
test_that("options_extract_metadata_true: Extract metadata returns document metadata when enabled", {
  ...
  expect_true(if (is.character(result$content)) length(result$content) > 0 && any(nchar(result$content) > 0) else length(result$content) > 0)
  expect_equal(result$metadata$document$title, "Test Page")
  expect_equal(result$metadata$document$description, "A test page")
})
...
test_that("options_output_format_plain: Plain text output format strips markdown syntax", {
  ...
  expect_true(any(grepl("text.", result$content, fixed = TRUE)))
  expect_false(grepl("**bold**", result$content, fixed = TRUE))
