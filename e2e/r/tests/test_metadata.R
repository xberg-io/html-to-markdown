...
# alef:hash:353f40c8c7d01d1b53b500a858cba26c3a32c04b968a609efb7e1549992cc9c9
...
test_that("metadata_author_meta: Extract author from <meta name='author'> tag", {
  ...
  expect_equal(result$metadata$document$author, "Jane Doe")
})
...
test_that("metadata_canonical_url: Extract canonical URL from <link rel='canonical'> tag", {
  ...
  expect_true(if (is.character(result$content)) length(result$content) > 0 && any(nchar(result$content) > 0) else length(result$content) > 0)
  expect_equal(result$metadata$document$canonical_url, "https://example.com/canonical-page")
})
...
test_that("metadata_description_meta: Extract description from <meta name='description'> tag", {
  ...
  expect_true(if (is.character(result$content)) length(result$content) > 0 && any(nchar(result$content) > 0) else length(result$content) > 0)
  expect_equal(result$metadata$document$description, "This is the page description.")
})
...
test_that("metadata_title_tag: Extract title from <title> tag", {
  ...
  expect_true(if (is.character(result$content)) length(result$content) > 0 && any(nchar(result$content) > 0) else length(result$content) > 0)
  expect_equal(result$metadata$document$title, "My Page")
})
...
test_that("og_basic_tags: Extract og:title, og:description, and og:image from Open Graph meta tags", {
  ...
  expect_true(if (is.character(result$content)) length(result$content) > 0 && any(nchar(result$content) > 0) else length(result$content) > 0)
  expect_equal(result$metadata$document$open_graph[["title"]], "OG Title")
  expect_equal(result$metadata$document$open_graph[["description"]], "OG description text.")
  expect_equal(result$metadata$document$open_graph[["image"]], "https://example.com/image.jpg")
})
...
test_that("og_multiple_tags: Extract multiple Open Graph tags including type, url, and site_name", {
  ...
  expect_true(if (is.character(result$content)) length(result$content) > 0 && any(nchar(result$content) > 0) else length(result$content) > 0)
  expect_equal(result$metadata$document$open_graph[["title"]], "Article Title")
  expect_equal(result$metadata$document$open_graph[["type"]], "article")
  expect_equal(result$metadata$document$open_graph[["url"]], "https://example.com/article")
  expect_equal(result$metadata$document$open_graph[["site_name"]], "Example Site")
})
...
test_that("structured_data_json_ld: JSON-LD script tag is stripped from output (security) but metadata may be extracted", {
  ...
  expect_false(grepl("application/ld+json", result$content, fixed = TRUE))
  expect_false(grepl("@context", result$content, fixed = TRUE))
  expect_false(grepl("schema.org", result$content, fixed = TRUE))
})
...
test_that("structured_data_multiple_json_ld: Multiple JSON-LD blocks are all stripped from output", {
  ...
  expect_false(grepl("@context", result$content, fixed = TRUE))
  expect_false(grepl("BreadcrumbList", result$content, fixed = TRUE))
  expect_false(grepl("ListItem", result$content, fixed = TRUE))
})
...
test_that("twitter_card_tags: Extract Twitter card meta tags", {
  ...
  expect_true(if (is.character(result$content)) length(result$content) > 0 && any(nchar(result$content) > 0) else length(result$content) > 0)
  expect_equal(result$metadata$document$twitter_card[["card"]], "summary_large_image")
  expect_equal(result$metadata$document$twitter_card[["title"]], "Twitter Card Title")
  expect_equal(result$metadata$document$twitter_card[["description"]], "Twitter card description.")
})
