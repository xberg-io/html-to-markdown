 ...
 * alef:hash:4aa6b9604fa65fd05cb8fb083336c1d86b3f66d393c96a5ff07b1d7031ad801e
...
void test_structure_code_block(void) {
  ...
  HTMAlefHandle result =
      htm_convert("<p>Example code:</p><pre><code class=\"language-rust\">fn "
                  "main() { println!(\"Hello\"); }</code></pre>",
                  options_handle);
  assert(result != 0 && "expected call to succeed");
  char *content = htm_conversion_result_content(result);
  HTMAlefHandle document_handle = htm_conversion_result_document(result);
  ...
  assert(document_nodes != NULL && strlen(document_nodes) > 0 &&
         strcmp(document_nodes, "[]") != 0 &&
         strcmp(document_nodes, "{}") != 0 && "expected non-empty value");
  {
...
void test_structure_h1_h2_nested_group(void) {
  ...
  HTMAlefHandle options_handle =
      htm_conversion_options_from_json("{\"include_document_structure\":true}");
  HTMAlefHandle result =
      htm_convert("<h1>Chapter One</h1><p>Chapter intro.</p><h2>Section "
                  "One</h2><p>Section content.</p>",
                  options_handle);
  assert(result != 0 && "expected call to succeed");
  char *content = htm_conversion_result_content(result);
  HTMAlefHandle document_handle = htm_conversion_result_document(result);
...
void test_structure_multiple_headings(void) {
  ...
  HTMAlefHandle options_handle =
      htm_conversion_options_from_json("{\"include_document_structure\":true}");
  HTMAlefHandle result =
      htm_convert("<h1>Main Title</h1><h2>Section One</h2><p>Section one "
...
void test_structure_sibling_h1_groups(void) {
  ...
  HTMAlefHandle options_handle =
      htm_conversion_options_from_json("{\"include_document_structure\":true}");
  HTMAlefHandle result =
      htm_convert("<h1>Chapter One</h1><h2>Section A</h2><p>Section A "
                  "content.</p><h1>Chapter Two</h1><h2>Section "
                  "B</h2><p>Section B content.</p>",
                  options_handle);
  assert(result != 0 && "expected call to succeed");
  char *content = htm_conversion_result_content(result);
  HTMAlefHandle document_handle = htm_conversion_result_document(result);
