 ...
 * alef:hash:b96d9371edcff9c12ba7a686bac4e3c77a43003192764d448c8e74db9a03b199
...
void test_result_tables_multiple(void) {
  ...
  HTMAlefHandle result = htm_convert(
      "<table><tr><th>A</th></tr><tr><td>1</td></tr></table><p>Between</"
...
void test_result_tables_without_structure_flag(void) {
  ...
  HTMAlefHandle result =
      htm_convert("<table><tr><th>X</th></tr><tr><td>Y</td></tr></table>", 0);
  assert(result != 0 && "expected call to succeed");
  char *content = htm_conversion_result_content(result);
  char *tables = htm_conversion_result_tables(result);
  assert(content != NULL && strlen(content) > 0 && strcmp(content, "[]") != 0 &&
...
void test_result_warning_kind_image_extraction_failed(void) {
  /* A malformed data URI with extract_images enabled produces an
   * ImageExtractionFailed warning */
  HTMAlefHandle options_handle =
      htm_conversion_options_from_json("{\"extract_images\":true}");
  HTMAlefHandle result =
      htm_convert("<p>Text<img src=\"data:BADMIME\" alt=\"broken\">end</p>",
                  options_handle);
  assert(result != 0 && "expected call to succeed");
  char *content = htm_conversion_result_content(result);
...
void test_result_warnings_empty_for_clean_input(void) {
  ...
  HTMAlefHandle result =
      htm_convert("<h1>Title</h1><p>Clean content with <a "
                  "href='https://example.com'>a link</a>.</p>",
                  0);
  assert(result != 0 && "expected call to succeed");
  char *content = htm_conversion_result_content(result);
  char *warnings = htm_conversion_result_warnings(result);
...
void test_result_warnings_empty_for_complex_input(void) {
  ...
  HTMAlefHandle result = htm_convert(
      "<article><h1>Article</h1><p>Paragraph with <strong>bold</strong> and "
...
void test_result_warnings_empty_for_malformed_html(void) {
  ...
  HTMAlefHandle result =
      htm_convert("<p>Unclosed paragraph<div>Mixed nesting</p></div>", 0);
  assert(result != 0 && "expected call to succeed");
  char *content = htm_conversion_result_content(result);
  char *warnings = htm_conversion_result_warnings(result);
  assert(content != NULL && strlen(content) > 0 && strcmp(content, "[]") != 0 &&
