...
// alef:hash:2d256e39b4dccff5b062abacb0a0e0a2dd40ef4c6d849c6e73f9d74878f9dc2d
...
fn test_metadata_author_meta() {
    ...
    assert_eq!(
        _metadata_document_author.to_string(),
        r#"Jane Doe"#,
...
fn test_metadata_canonical_url() {
    ...
    assert!(result.content.is_some(), "expected content to be present");
    assert_eq!(
        _metadata_document_canonical_url.to_string(),
...
fn test_metadata_dublin_core() {
    ...
    assert!(
        _content.contains(r#"scholarly article"#),
        "expected to contain: {}",
        r#"scholarly article"#
    );
}
...
fn test_metadata_lang_attribute() {
    ...
    assert!(!_content.is_empty(), "expected non-empty value");
    assert!(
        _content.contains(r#"Hola Mundo"#),
        "expected to contain: {}",
        r#"Hola Mundo"#
    );
}
...
fn test_metadata_microdata_schema_article() {
    ...
    assert!(!_content.is_empty(), "expected non-empty value");
    assert!(
        _content.contains(r#"Breaking News Today"#),
        "expected to contain: {}",
        r#"Breaking News Today"#
    );
    assert!(
        _content.contains(r#"Jane Reporter"#),
        "expected to contain: {}",
        r#"Jane Reporter"#
    );
    assert!(
        _content.contains(r#"important information"#),
        "expected to contain: {}",
...
fn test_metadata_microdata_schema_breadcrumb() {
    ...
    let _content = result.content.as_ref().map(|v| v.to_string()).unwrap_or_default();
    assert!(!_content.is_empty(), "expected non-empty value");
    assert!(_content.contains(r#"Home"#), "expected to contain: {}", r#"Home"#);
    assert!(
        _content.contains(r#"Products"#),
        "expected to contain: {}",
        r#"Products"#
    );
    assert!(
        _content.contains(r#"Current Page"#),
        "expected to contain: {}",
        r#"Current Page"#
    );
}
...
fn test_metadata_microdata_schema_organization() {
    ...
    assert!(!_content.is_empty(), "expected non-empty value");
    assert!(
        _content.contains(r#"Acme Corp"#),
        "expected to contain: {}",
        r#"Acme Corp"#
    );
    assert!(_content.contains(r#"2020"#), "expected to contain: {}", r#"2020"#);
}
...
fn test_metadata_microdata_schema_person() {
    ...
    assert!(!_content.is_empty(), "expected non-empty value");
    assert!(
        _content.contains(r#"John Smith"#),
        "expected to contain: {}",
        r#"John Smith"#
    );
    assert!(
        _content.contains(r#"john@example.com"#),
        "expected to contain: {}",
        r#"john@example.com"#
    );
}
...
fn test_metadata_microdata_schema_product() {
    ...
    assert!(!_content.is_empty(), "expected non-empty value");
    assert!(
        _content.contains(r#"Awesome Widget"#),
        "expected to contain: {}",
        r#"Awesome Widget"#
    );
    assert!(
        _content.contains(r#"best widget"#),
        "expected to contain: {}",
        r#"best widget"#
    );
    assert!(_content.contains(r#"29.99"#), "expected to contain: {}", r#"29.99"#);
}
...
fn test_metadata_text_direction_ltr() {
    ...
    assert!(!_content.is_empty(), "expected non-empty value");
    assert!(
        _content.contains(r#"left-to-right text"#),
        "expected to contain: {}",
...
fn test_metadata_text_direction_rtl() {
    ...
    assert!(!_content.is_empty(), "expected non-empty value");
    assert!(
        _content.contains(r#"right-to-left text"#),
        "expected to contain: {}",
...
fn test_og_basic_tags() {
    ...
    assert!(result.content.is_some(), "expected content to be present");
    assert_eq!(
        _metadata_document_open_graph_title.to_string(),
        r#"OG Title"#,
        "equals assertion failed"
    );
    assert_eq!(
        _metadata_document_open_graph_description.to_string(),
        r#"OG description text."#,
        "equals assertion failed"
    );
    assert_eq!(
        _metadata_document_open_graph_image.to_string(),
...
fn test_og_multiple_tags() {
    ...
    assert_eq!(
        _metadata_document_open_graph_type.to_string(),
        r#"article"#,
        "equals assertion failed"
    );
    assert_eq!(
        _metadata_document_open_graph_url.to_string(),
        r#"https://example.com/article"#,
        "equals assertion failed"
    );
    assert_eq!(
        _metadata_document_open_graph_site_name.to_string(),
        r#"Example Site"#,
        "equals assertion failed"
    );
}
...
fn test_structured_data_json_ld() {
    ...
    assert!(!_content.is_empty(), "expected non-empty value");
    assert!(
        _content.contains(r#"My Article"#),
        "expected to contain: {}",
        r#"My Article"#
    );
    assert!(
        !_content.contains(r#"application/ld+json"#),
        "expected NOT to contain: {}",
        r#"application/ld+json"#
    );
    assert!(
        !_content.contains(r#"@context"#),
        "expected NOT to contain: {}",
        r#"@context"#
    );
    assert!(
        !_content.contains(r#"schema.org"#),
        "expected NOT to contain: {}",
        r#"schema.org"#
    );
}
...
fn test_structured_data_multiple_json_ld() {
    ...
    let _content = result.content.as_ref().map(|v| v.to_string()).unwrap_or_default();
    assert!(!_content.is_empty(), "expected non-empty value");
    assert!(_content.contains(r#"Widget"#), "expected to contain: {}", r#"Widget"#);
    assert!(
        !_content.contains(r#"@context"#),
        "expected NOT to contain: {}",
        r#"@context"#
    ...
    assert!(
        !_content.contains(r#"BreadcrumbList"#),
        "expected NOT to contain: {}",
        r#"BreadcrumbList"#
    );
    assert!(
        !_content.contains(r#"ListItem"#),
        "expected NOT to contain: {}",
        r#"ListItem"#
    );
}
...
fn test_twitter_card_tags() {
    ...
    assert!(result.content.is_some(), "expected content to be present");
    assert_eq!(
        _metadata_document_twitter_card_card.to_string(),
        r#"summary_large_image"#,
        "equals assertion failed"
    );
    assert_eq!(
        _metadata_document_twitter_card_title.to_string(),
        r#"Twitter Card Title"#,
        "equals assertion failed"
    );
    assert_eq!(
        _metadata_document_twitter_card_description.to_string(),
