...
// alef:hash:34c508639dd2d0ccd1edbec39d0998b56127f1868f890713153b27c446c4a3dc
...
fn test_metadata_author_meta() {
    ...
    assert_eq!(
        format!("{:?}", metadata_document_author).as_str().trim(),
        r#"Jane Doe"#,
...
fn test_metadata_canonical_url() {
    ...
    assert!(result.content.is_some(), "expected content to be present");
    assert_eq!(
        format!("{:?}", metadata_document_canonical_url).as_str().trim(),
...
fn test_og_basic_tags() {
    ...
    assert!(result.content.is_some(), "expected content to be present");
    assert_eq!(
        format!("{:?}", metadata_document_open_graph_title).as_str().trim(),
        r#"OG Title"#,
        "equals assertion failed"
    );
    assert_eq!(
        format!("{:?}", metadata_document_open_graph_description)
            .as_str()
            .trim(),
    ...
    assert_eq!(
        format!("{:?}", metadata_document_open_graph_image).as_str().trim(),
...
fn test_og_multiple_tags() {
    ...
    assert_eq!(
        format!("{:?}", metadata_document_open_graph_type).as_str().trim(),
        r#"article"#,
        "equals assertion failed"
    );
    assert_eq!(
        format!("{:?}", metadata_document_open_graph_url).as_str().trim(),
        r#"https://example.com/article"#,
        "equals assertion failed"
    );
    assert_eq!(
        format!("{:?}", metadata_document_open_graph_site_name).as_str().trim(),
        r#"Example Site"#,
        "equals assertion failed"
    );
}
...
fn test_twitter_card_tags() {
    ...
    assert!(result.content.is_some(), "expected content to be present");
    assert_eq!(
        format!("{:?}", metadata_document_twitter_card_card).as_str().trim(),
        r#"summary_large_image"#,
        "equals assertion failed"
    );
    assert_eq!(
        format!("{:?}", metadata_document_twitter_card_title).as_str().trim(),
        r#"Twitter Card Title"#,
        "equals assertion failed"
    );
    assert_eq!(
        format!("{:?}", metadata_document_twitter_card_description)
            .as_str()
            .trim(),
