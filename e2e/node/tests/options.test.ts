...
describe('options', () => {  it('issue_396_backticks_blank_line_after_fence: Backticks code block trims trailing newline inside fence and adds blank line after closing fence (issue #396)', () => {    const result = convert("<p>Foo</p><pre><code>1\n2\n</code></pre><p>Bar</p>", { codeBlockStyle: "Backticks" } as ConversionOptions);    expect((result.content ?? "").trim()).toBe("Foo\n\n```\n1\n2\n```\n\nBar");

  }, 30000);
  it('options_autolinks_false: Bare URL links rendered as regular markdown links when autolinks disabled', () => {    const result = convert("<p><a href='https://example.com'>https://example.com</a></p>", { autolinks: false } as ConversionOptions);    expect(result.content).toContain("example.com");
