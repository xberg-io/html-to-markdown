...
// alef:hash:54450476c39b5a3fb67944f11ef1a17ab76d269d976e7cae5d96152c0f15f930
...
describe("conversion", () => {
  ...
  it("blockquote_simple: Simple blockquote", () => {
    const result = convert("<blockquote><p>Quote text</p></blockquote>", {} as ConversionOptions);
  ...
  it("bold_and_italic: Nested bold and italic", () => {
    const result = convert("<p><strong><em>both</em></strong></p>", {} as ConversionOptions);
    expect(result.content).toContain("***both***");
  });

  it("bold_strong: Strong tag converts to bold", () => {
    const result = convert("<p><strong>bold</strong></p>", {} as ConversionOptions);
  ...
  it("code_block_no_language: Code block without a language class preserves content", () => {
    const result = convert("<pre><code>plain code here</code></pre>", {} as ConversionOptions);
    expect((result.content ?? "").length).toBeGreaterThan(0);
  ...
  it("emphasis_superscript: sup tag content is preserved", () => {
    const result = convert("<p>x<sup>2</sup></p>", {} as ConversionOptions);
    expect(result.content).toContain("x");
    expect(result.content).toContain("2");
  });

  it("emphasis_underline_u: u tag content is preserved in output", () => {
    const result = convert("<p><u>underlined</u></p>", {} as ConversionOptions);
  ...
  it("heading_h1: H1 heading", () => {
    const result = convert("<h1>Heading 1</h1>", {} as ConversionOptions);
    expect((result.content ?? "").trim()).toBe("# Heading 1");
  });

  it("heading_h2: H2 heading", () => {
    const result = convert("<h2>Heading 2</h2>", {} as ConversionOptions);
    expect((result.content ?? "").trim()).toBe("## Heading 2");
  });

  it("heading_h3: H3 heading", () => {
    const result = convert("<h3>Heading 3</h3>", {} as ConversionOptions);
    expect((result.content ?? "").trim()).toBe("### Heading 3");
  });

  it("heading_h4: H4 heading", () => {
    const result = convert("<h4>Heading 4</h4>", {} as ConversionOptions);
    expect((result.content ?? "").trim()).toBe("#### Heading 4");
  });

  it("heading_h5: H5 heading", () => {
    const result = convert("<h5>Heading 5</h5>", {} as ConversionOptions);
    expect((result.content ?? "").trim()).toBe("##### Heading 5");
  });

  it("heading_h6: H6 heading", () => {
    const result = convert("<h6>Heading 6</h6>", {} as ConversionOptions);
  ...
  it("image_no_alt: Image without alt text produces image markdown", () => {
    const result = convert('<img src="banner.jpg">', {} as ConversionOptions);
    expect((result.content ?? "").length).toBeGreaterThan(0);
    expect(result.content).toContain("banner.jpg");
  });

  it("image_simple: Image with alt text", () => {
    const result = convert('<img src="photo.jpg" alt="A photo">', {} as ConversionOptions);
  ...
  it("italic_em: Em tag converts to italic", () => {
    const result = convert("<p><em>italic</em></p>", {} as ConversionOptions);
    expect(result.content).toContain("*italic*");
  });

  it("line_break_br_tag: Single br tag produces a line break in output", () => {
    const result = convert("<p>First line.<br>Second line.</p>", {} as ConversionOptions);
  ...
  it("line_break_hr_tag: hr tag produces a horizontal separator between content", () => {
    const result = convert("<p>Before rule.</p><hr><p>After rule.</p>", {} as ConversionOptions);
    expect((result.content ?? "").length).toBeGreaterThan(0);
  ...
  it("line_break_multiple_br: Multiple consecutive br tags in sequence", () => {
    const result = convert("<p>Start.<br><br>End.</p>", {} as ConversionOptions);
    expect(result.content).toContain("Start.");
    expect(result.content).toContain("End.");
  });

  it("link_anchor_fragment: Fragment-only anchor link is preserved", () => {
    const result = convert('<a href="#section">Jump to section</a>', {} as ConversionOptions);
  ...
  it("link_empty_href: Link with empty href produces output with the link text", () => {
    const result = convert('<a href="">No destination</a>', {} as ConversionOptions);
  ...
  it("link_simple: Simple link", () => {
    const result = convert('<a href="https://example.com">Example</a>', {} as ConversionOptions);
  ...
  it("paragraph_nested_divs: Text nested inside divs is extracted correctly", () => {
    const result = convert("<div><div><p>Nested text</p></div></div>", {} as ConversionOptions);
  ...
  it("paragraph_with_line_breaks: Paragraph with br tags produces line breaks in output", () => {
    const result = convert("<p>Line one.<br>Line two.<br>Line three.</p>", {} as ConversionOptions);
    expect((result.content ?? "").length).toBeGreaterThan(0);
  ...
  it("semantic_hr: Horizontal rule produces a separator in output", () => {
    const result = convert("<p>Above</p><hr><p>Below</p>", {} as ConversionOptions);
    expect((result.content ?? "").length).toBeGreaterThan(0);
  ...
  it("semantic_sub_superscript: Subscript and superscript elements are preserved in output", () => {
    const result = convert("<p>H<sub>2</sub>O and E=mc<sup>2</sup></p>", {} as ConversionOptions);
    expect((result.content ?? "").length).toBeGreaterThan(0);
    expect(result.content).toContain("H");
    expect(result.content).toContain("2");
    expect(result.content).toContain("O");
  ...
  it("table_empty: Empty table produces no output or minimal output", () => {
    const result = convert("<table></table>", {} as ConversionOptions);
