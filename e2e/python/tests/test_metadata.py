...
# alef:hash:370a9eee8e491583dfdba6838b429c9fbb37cf6b095804e0d96881fc414525ad
...
def test_metadata_dublin_core() -> None:
    ...
    assert "scholarly article" in result.content
...
def test_metadata_microdata_schema_article() -> None:
    ...
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "Breaking News Today" in result.content
    assert result.content is not None
    assert "Jane Reporter" in result.content
    assert result.content is not None
...
def test_metadata_microdata_schema_breadcrumb() -> None:
    ...
    options = ConversionOptions(extract_metadata=True, preprocessing=PreprocessingOptions(remove_navigation=False))
    ...
    assert result.content is not None
    assert "Current Page" in result.content
...
def test_metadata_microdata_schema_product() -> None:
    ...
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "Awesome Widget" in result.content
    assert result.content is not None
    assert "best widget" in result.content
    assert result.content is not None
...
def test_metadata_text_direction_ltr() -> None:
    ...
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "left-to-right text" in result.content
...
def test_metadata_text_direction_rtl() -> None:
    ...
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.content is not None
    assert "right-to-left text" in result.content
...
def test_og_basic_tags() -> None:
    ...
    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.metadata.document.open_graph.get("title") == "OG Title"
    assert result.metadata.document.open_graph.get("description") == "OG description text."
    assert result.metadata.document.open_graph.get("image") == "https://example.com/image.jpg"
...
def test_og_multiple_tags() -> None:
    ...
    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.metadata.document.open_graph.get("title") == "Article Title"
    assert result.metadata.document.open_graph.get("type") == "article"
    assert result.metadata.document.open_graph.get("url") == "https://example.com/article"
    assert result.metadata.document.open_graph.get("site_name") == "Example Site"
...
def test_structured_data_json_ld() -> None:
    ...
    assert result.content is None or "application/ld+json" not in result.content
    assert result.content is None or "@context" not in result.content
    assert result.content is None or "schema.org" not in result.content
...
def test_structured_data_multiple_json_ld() -> None:
    ...
    assert result.content is None or "@context" not in result.content
    assert result.content is None or "BreadcrumbList" not in result.content
    assert result.content is None or "ListItem" not in result.content
...
def test_twitter_card_tags() -> None:
    ...
    result = convert(html)
    assert result.content is not None and (not hasattr(result.content, "__len__") or len(result.content) > 0)
    assert result.metadata.document.twitter_card.get("card") == "summary_large_image"
    assert result.metadata.document.twitter_card.get("title") == "Twitter Card Title"
    assert result.metadata.document.twitter_card.get("description") == "Twitter card description."
