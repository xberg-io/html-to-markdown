...
// alef:hash:454dba3ed14b18a3df6af556756c34dd9d64caa5e6f0607f2df0e81b4c061179
...
describe("options", () => {
  ...
  it("options_br_in_tables_anchor_wrapped_nested_table_in_layout_cell: An anchor-wrapped table inside a layout cell keeps its inner links clickable", () => {
    const result = convert(
    ...
    expect(result.content ?? "").toBe("- [/outer](/outer)\n\n| [One](/one)[Two](/two) |\n| --- |\n- After\n");
  }, 30000);
  ...
  it("options_br_in_tables_nested_data_table: A single-cell layout wrapper preserves its nested data table as structured Markdown", () => {
    const result = convert(
  ...
  it("options_br_in_tables_nested_table_rows: A nested table flattened into a cell keeps its row boundaries as literal <br> when br_in_tables is enabled", () => {
    const result = convert(
    ...
    expect(result.content).toContain("Before<br>");
  ...
  it("options_custom_bullets_apply_to_layout_table_rows: A layout table renders its rows as list items, and those markers honour the bullets option like any other list", () => {
    const result = convert(
  ...
  it("options_extract_metadata_true: Extract metadata returns document metadata when enabled", () => {
    ...
    expect(result.metadata.document.description ?? "").toBe("A test page");
  }, 30000);
