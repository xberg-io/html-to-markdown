...
// alef:hash:14dd99b7ef1fccbfc49ad3697dbcbaf1d4c8a63d38320d5fc76a62537fcb7ee0
...
describe("metadata", () => {
  it("metadata_author_meta: Extract author from <meta name='author'> tag", () => {
    ...
    expect(result.metadata.document.author ?? "").toBe("Jane Doe");
  }, 30000);
  ...
  it("metadata_canonical_url: Extract canonical URL from <link rel='canonical'> tag", () => {
    ...
    expect(result.metadata.document.canonicalUrl ?? "").toBe("https://example.com/canonical-page");
  }, 30000);
  ...
  it("metadata_description_meta: Extract description from <meta name='description'> tag", () => {
    ...
    expect(result.metadata.document.description ?? "").toBe("This is the page description.");
  }, 30000);
  ...
  it("metadata_title_tag: Extract title from <title> tag", () => {
    ...
    expect(result.metadata.document.title ?? "").toBe("My Page");
  }, 30000);
  ...
  it("og_basic_tags: Extract og:title, og:description, and og:image from Open Graph meta tags", () => {
    ...
    expect(result.metadata.document.openGraph["title"] ?? "").toBe("OG Title");
    expect(result.metadata.document.openGraph["description"] ?? "").toBe("OG description text.");
    expect(result.metadata.document.openGraph["image"] ?? "").toBe("https://example.com/image.jpg");
  }, 30000);
  ...
  it("og_multiple_tags: Extract multiple Open Graph tags including type, url, and site_name", () => {
    ...
    expect(result.metadata.document.openGraph["title"] ?? "").toBe("Article Title");
    expect(result.metadata.document.openGraph["type"] ?? "").toBe("article");
    expect(result.metadata.document.openGraph["url"] ?? "").toBe("https://example.com/article");
    expect(result.metadata.document.openGraph["site_name"] ?? "").toBe("Example Site");
  }, 30000);
  ...
  it("structured_data_json_ld: JSON-LD script tag is stripped from output (security) but metadata may be extracted", () => {
    ...
    expect(result.content).not.toContain("schema.org");
  }, 30000);
  ...
  it("structured_data_multiple_json_ld: Multiple JSON-LD blocks are all stripped from output", () => {
    ...
    expect(result.content).not.toContain("ListItem");
  }, 30000);
  ...
  it("twitter_card_tags: Extract Twitter card meta tags", () => {
    ...
    expect(result.metadata.document.twitterCard["card"] ?? "").toBe("summary_large_image");
    expect(result.metadata.document.twitterCard["title"] ?? "").toBe("Twitter Card Title");
    expect(result.metadata.document.twitterCard["description"] ?? "").toBe("Twitter card description.");
  }, 30000);
});
