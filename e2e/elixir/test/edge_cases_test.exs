...
# alef:hash:21779e0820499185a3a82465fbbf0a5290be628b29277ae60d4ac80dd82aa1ef
...
defmodule E2e.EdgeCasesTest do
  ...
  describe "empty_html" do
    test "Empty HTML document" do
      {:ok, result} = HtmlToMarkdownRs.convert("<html><head></head><body></body></html>")
  ...
  describe "encoding_cjk_characters" do
    test "CJK (Chinese, Japanese, Korean) characters are preserved" do
      {:ok, result} = HtmlToMarkdownRs.convert("<p>中文内容</p><p>日本語テキスト</p><p>한국어 텍스트</p>")
  ...
  describe "encoding_html_entities" do
    test "Common HTML entities are decoded in output" do
      {:ok, result} = HtmlToMarkdownRs.convert("<p>&amp; &lt; &gt; &nbsp; &quot; &apos;</p>")
      assert result.content != ""
  ...
  describe "encoding_named_entities" do
    test "Named HTML entities like &mdash; and &hellip; are decoded" do
      {:ok, result} = HtmlToMarkdownRs.convert("<p>Em dash&mdash;used for parenthetical remarks&mdash;is common. Ellipsis&hellip; indicates omission. Non-breaking&nbsp;space.</p>")
      assert result.content != ""
      assert String.contains?(to_string(result.content), "—")
      assert String.contains?(to_string(result.content), "…")
    end
  ...
  describe "encoding_numeric_entities" do
    test "Numeric HTML entities (decimal and hex) are decoded" do
      {:ok, result} = HtmlToMarkdownRs.convert("<p>Copyright: &\#169; Trade: &\#174; Euro: &\#8364; Hex: &\#x00A9;</p>")
      assert result.content != ""
      assert String.contains?(to_string(result.content), "©")
      assert String.contains?(to_string(result.content), "®")
      assert String.contains?(to_string(result.content), "€")
  ...
  describe "encoding_unicode_emoji" do
    test "Emoji and Unicode characters are preserved" do
      {:ok, result} = HtmlToMarkdownRs.convert("<p>Hello 🌍 World 🚀</p><p>Stars: ⭐ ✨</p>")
      assert result.content != ""
      assert String.contains?(to_string(result.content), "🌍")
      assert String.contains?(to_string(result.content), "🚀")
      assert String.contains?(to_string(result.content), "⭐")
  ...
  describe "html_comments_only" do
    test "Document containing only HTML comments produces empty output" do
      {:ok, result} = HtmlToMarkdownRs.convert("<!-- This is a comment --><!-- Another comment -->")
      assert String.trim(result.content) == ""
    end
  end
  ...
  describe "malformed_deeply_nested_elements" do
    test "Deeply nested elements (100 levels) are handled without stack overflow" do
      {:ok, result} = HtmlToMarkdownRs.convert("<div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><p>Deeply nested content</p></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div>")
      assert result.content != ""
  ...
  describe "malformed_missing_block_closing_tags" do
    test "Missing closing tags on block elements are auto-closed by parser" do
      {:ok, result} = HtmlToMarkdownRs.convert("<div><h1>Title<p>First paragraph<p>Second paragraph</div>")
      assert result.content != ""
  ...
  describe "malformed_overlapping_tags" do
    test "Overlapping bold/italic tags are recovered by the HTML parser without panic" do
      {:ok, result} = HtmlToMarkdownRs.convert("<p><b><i>bold and italic</b></i></p>")
      assert result.content != ""
  ...
  describe "malformed_unclosed_paragraph" do
    test "Unclosed <p> tag is recovered gracefully and content is preserved" do
      {:ok, result} = HtmlToMarkdownRs.convert("<p>This paragraph is never closed")
      assert result.content != ""
  ...
  describe "script_tags_only" do
    test "Document with only script tags produces empty output (scripts are stripped)" do
      {:ok, result} = HtmlToMarkdownRs.convert("<html><head><script>alert('xss')</script></head><body><script>document.write('hello')</script></body></html>")
      assert String.trim(result.content) == ""
    end
  end
  ...
  describe "style_tags_only" do
    test "Document with only style tags produces empty output (styles are stripped)" do
      {:ok, result} = HtmlToMarkdownRs.convert("<html><head><style>body { color: red; }</style></head><body><style>.foo { margin: 0; }</style></body></html>")
      assert String.trim(result.content) == ""
    end
  end
  ...
  describe "visitor_custom_element_with_nesting" do
    test "Visitor handles custom elements with nested content" do
      ...
      {:ok, result} = HtmlToMarkdownRs.convert("<div><custom-widget data-value=\"123\"><p>Widget content here</p><span>With nested elements</span></custom-widget></div>", visitor)
  ...
  describe "visitor_deeply_nested_skip" do
    test "Visitor skips deeply nested elements" do
      ...
      {:ok, result} = HtmlToMarkdownRs.convert("<div><p>Outer <em>emphasis <strong>with bold <mark>and highlight</mark></strong></em> text</p></div>", visitor)
  ...
  describe "visitor_element_end_modification" do
    test "Visitor modifies element at end after children processed" do
      ...
      {:ok, result} = HtmlToMarkdownRs.convert("<blockquote><p>Original quote</p></blockquote>", visitor)
      assert result.content != ""
    end
  end
  ...
  describe "visitor_element_start_skip_entire_subtree" do
    test "Visitor skips at element_start level removes entire subtree" do
      ...
      {:ok, result} = HtmlToMarkdownRs.convert("<div><h1>Title</h1><p>Content</p></div>", visitor)
      assert result.content != ""
    end
  end
  ...
  describe "visitor_unknown_tag_preservation" do
    test "Visitor preserves unknown HTML tags as raw HTML" do
      ...
      {:ok, result} = HtmlToMarkdownRs.convert("<article><p>Article text</p><x-custom>Custom element with content</x-custom><p>More article text</p></article>", visitor)
  ...
  describe "xss_onclick_handler_removed" do
    test "onclick and other on* event handlers are removed from elements" do
      {:ok, result} = HtmlToMarkdownRs.convert("<p><a href=\"https://example.com\" onclick=\"alert('xss')\">Click me</a></p><button onmouseover=\"steal_data()\">Hover me</button>")
      assert result.content != ""
  ...
  describe "xss_script_tag_stripped" do
    test "Script tag content is stripped and does not appear in output" do
      {:ok, result} = HtmlToMarkdownRs.convert("<p>Safe content.</p><script>alert('xss')</script><p>More safe content.</p>")
      assert result.content != ""
  ...
  describe "xss_svg_nested_script_stripped" do
    test "Script tags nested inside SVG are stripped" do
      {:ok, result} = HtmlToMarkdownRs.convert("<p>Before SVG.</p><svg xmlns=\"http://www.w3.org/2000/svg\"><script>alert('svg-xss')</script><text>SVG text</text></svg><p>After SVG.</p>")
      assert result.content != ""
