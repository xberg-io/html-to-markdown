...
# alef:hash:17822f72ee21e8a48cb90ec64cbaaa8f1311e00fb8b8c69ce059ad7adef90df4
...
RSpec.describe "metadata" do
  ...
  it "metadata_author_meta: Extract author from <meta name='author'> tag" do
    ...
    expect(result.metadata.document.author.to_s).to(eq("Jane Doe"))
  end
  ...
  it "metadata_canonical_url: Extract canonical URL from <link rel='canonical'> tag" do
    ...
    expect(result.content.respond_to?(:empty?) ? !result.content.empty? : !result.content.nil?).to(be(true))
    expect(result.metadata.document.canonical_url.to_s).to(eq("https://example.com/canonical-page"))
  end
  ...
  it "metadata_description_meta: Extract description from <meta name='description'> tag" do
    ...
    expect(result.content.respond_to?(:empty?) ? !result.content.empty? : !result.content.nil?).to(be(true))
    expect(result.metadata.document.description.to_s).to(eq("This is the page description."))
  end
  ...
  it "metadata_microdata_schema_breadcrumb: Extract schema.org breadcrumb navigation microdata" do
    result = HtmlToMarkdown.convert(
      ...
      HtmlToMarkdown::ConversionOptions.new(extract_metadata: true, preprocessing: {"remove_navigation" => false})
    )
    expect(result.content.respond_to?(:empty?) ? !result.content.empty? : !result.content.nil?).to(be(true))
  ...
  it "metadata_title_tag: Extract title from <title> tag" do
    ...
    expect(result.content.respond_to?(:empty?) ? !result.content.empty? : !result.content.nil?).to(be(true))
    expect(result.metadata.document.title.to_s).to(eq("My Page"))
  end
  ...
  it "og_basic_tags: Extract og:title, og:description, and og:image from Open Graph meta tags" do
    ...
    expect(result.content.respond_to?(:empty?) ? !result.content.empty? : !result.content.nil?).to(be(true))
    expect(result.metadata.document.open_graph["title"].to_s).to(eq("OG Title"))
    expect(result.metadata.document.open_graph["description"].to_s).to(eq("OG description text."))
    expect(result.metadata.document.open_graph["image"].to_s).to(eq("https://example.com/image.jpg"))
  end
  ...
  it "og_multiple_tags: Extract multiple Open Graph tags including type, url, and site_name" do
    ...
    expect(result.content.respond_to?(:empty?) ? !result.content.empty? : !result.content.nil?).to(be(true))
    expect(result.metadata.document.open_graph["title"].to_s).to(eq("Article Title"))
    expect(result.metadata.document.open_graph["type"].to_s).to(eq("article"))
    expect(result.metadata.document.open_graph["url"].to_s).to(eq("https://example.com/article"))
    expect(result.metadata.document.open_graph["site_name"].to_s).to(eq("Example Site"))
  end
  ...
  it "structured_data_json_ld: JSON-LD script tag is stripped from output (security) but metadata may be extracted" do
    ...
    expect(result.content.to_s).not_to(include("application/ld+json"))
    expect(result.content.to_s).not_to(include("@context"))
    expect(result.content.to_s).not_to(include("schema.org"))
  end
  ...
  it "structured_data_multiple_json_ld: Multiple JSON-LD blocks are all stripped from output" do
    ...
    expect(result.content.to_s).not_to(include("@context"))
    expect(result.content.to_s).not_to(include("BreadcrumbList"))
    expect(result.content.to_s).not_to(include("ListItem"))
  end
  ...
  it "twitter_card_tags: Extract Twitter card meta tags" do
    ...
    expect(result.content.respond_to?(:empty?) ? !result.content.empty? : !result.content.nil?).to(be(true))
    expect(result.metadata.document.twitter_card["card"].to_s).to(eq("summary_large_image"))
    expect(result.metadata.document.twitter_card["title"].to_s).to(eq("Twitter Card Title"))
    expect(result.metadata.document.twitter_card["description"].to_s).to(eq("Twitter card description."))
  end
end
