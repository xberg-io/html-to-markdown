...
# alef:hash:85dd987dd96c421d019a42dc65fee679daa928ba6e373445d60a833a5c8fcf8a
...
RSpec.describe 'edge-cases' do
  it 'empty_html: Empty HTML document' do
    result = HtmlToMarkdownRs.convert('<html><head></head><body></body></html>')
    expect(result.strip).to eq('')
  end

  it 'encoding_cjk_characters: CJK (Chinese, Japanese, Korean) characters are preserved' do
    result = HtmlToMarkdownRs.convert('<p>中文内容</p><p>日本語テキスト</p><p>한국어 텍스트</p>')
  ...
  it 'encoding_html_entities: Common HTML entities are decoded in output' do
    result = HtmlToMarkdownRs.convert('<p>&amp; &lt; &gt; &nbsp; &quot; &apos;</p>')
    expect(result).not_to be_empty
  ...
  it 'encoding_named_entities: Named HTML entities like &mdash; and &hellip; are decoded' do
    result = HtmlToMarkdownRs.convert('<p>Em dash&mdash;used for parenthetical remarks&mdash;is common. Ellipsis&hellip; indicates omission. Non-breaking&nbsp;space.</p>')
    expect(result).not_to be_empty
    expect(result.to_s).to include('—')
    expect(result.to_s).to include('…')
  end
  ...
  it 'encoding_numeric_entities: Numeric HTML entities (decimal and hex) are decoded' do
    result = HtmlToMarkdownRs.convert('<p>Copyright: &#169; Trade: &#174; Euro: &#8364; Hex: &#x00A9;</p>')
    expect(result).not_to be_empty
    expect(result.to_s).to include('©')
    expect(result.to_s).to include('®')
    expect(result.to_s).to include('€')
  ...
  it 'encoding_unicode_emoji: Emoji and Unicode characters are preserved' do
    result = HtmlToMarkdownRs.convert('<p>Hello 🌍 World 🚀</p><p>Stars: ⭐ ✨</p>')
    expect(result).not_to be_empty
    expect(result.to_s).to include('🌍')
    expect(result.to_s).to include('🚀')
    expect(result.to_s).to include('⭐')
  ...
  it 'html_comments_only: Document containing only HTML comments produces empty output' do
    result = HtmlToMarkdownRs.convert('<!-- This is a comment --><!-- Another comment -->')
    expect(result.strip).to eq('')
  end
  ...
  it 'malformed_deeply_nested_elements: Deeply nested elements (100 levels) are handled without stack overflow' do
    result = HtmlToMarkdownRs.convert('<div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><div><p>Deeply nested content</p></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div>')
    expect(result).not_to be_empty
  ...
  it 'malformed_missing_block_closing_tags: Missing closing tags on block elements are auto-closed by parser' do
    result = HtmlToMarkdownRs.convert('<div><h1>Title<p>First paragraph<p>Second paragraph</div>')
    expect(result).not_to be_empty
  ...
  it 'malformed_overlapping_tags: Overlapping bold/italic tags are recovered by the HTML parser without panic' do
    result = HtmlToMarkdownRs.convert('<p><b><i>bold and italic</b></i></p>')
    expect(result).not_to be_empty
  ...
  it 'malformed_unclosed_paragraph: Unclosed <p> tag is recovered gracefully and content is preserved' do
    result = HtmlToMarkdownRs.convert('<p>This paragraph is never closed')
    expect(result).not_to be_empty
  ...
  it 'script_tags_only: Document with only script tags produces empty output (scripts are stripped)' do
    result = HtmlToMarkdownRs.convert('<html><head><script>alert(\'xss\')</script></head><body><script>document.write(\'hello\')</script></body></html>')
    expect(result.strip).to eq('')
  end

  it 'style_tags_only: Document with only style tags produces empty output (styles are stripped)' do
    result = HtmlToMarkdownRs.convert('<html><head><style>body { color: red; }</style></head><body><style>.foo { margin: 0; }</style></body></html>')
    expect(result.strip).to eq('')
  end
  ...
  it 'visitor_custom_element_with_nesting: Visitor handles custom elements with nested content' do
    ...
    result = HtmlToMarkdownRs.convert('<div><custom-widget data-value="123"><p>Widget content here</p><span>With nested elements</span></custom-widget></div>', visitor)
  ...
  it 'visitor_deeply_nested_skip: Visitor skips deeply nested elements' do
    ...
    result = HtmlToMarkdownRs.convert('<div><p>Outer <em>emphasis <strong>with bold <mark>and highlight</mark></strong></em> text</p></div>', visitor)
  ...
  it 'visitor_element_end_modification: Visitor modifies element at end after children processed' do
    ...
    result = HtmlToMarkdownRs.convert('<blockquote><p>Original quote</p></blockquote>', visitor)
    expect(result).not_to be_empty
  end
  ...
  it 'visitor_element_start_skip_entire_subtree: Visitor skips at element_start level removes entire subtree' do
    ...
    result = HtmlToMarkdownRs.convert('<div><h1>Title</h1><p>Content</p></div>', visitor)
    expect(result).not_to be_empty
  end
  ...
  it 'visitor_unknown_tag_preservation: Visitor preserves unknown HTML tags as raw HTML' do
    ...
    result = HtmlToMarkdownRs.convert('<article><p>Article text</p><x-custom>Custom element with content</x-custom><p>More article text</p></article>', visitor)
  ...
  it 'xss_onclick_handler_removed: onclick and other on* event handlers are removed from elements' do
    result = HtmlToMarkdownRs.convert('<p><a href="https://example.com" onclick="alert(\'xss\')">Click me</a></p><button onmouseover="steal_data()">Hover me</button>')
    expect(result).not_to be_empty
  ...
  it 'xss_script_tag_stripped: Script tag content is stripped and does not appear in output' do
    result = HtmlToMarkdownRs.convert('<p>Safe content.</p><script>alert(\'xss\')</script><p>More safe content.</p>')
    expect(result).not_to be_empty
  ...
  it 'xss_svg_nested_script_stripped: Script tags nested inside SVG are stripped' do
    result = HtmlToMarkdownRs.convert('<p>Before SVG.</p><svg xmlns="http://www.w3.org/2000/svg"><script>alert(\'svg-xss\')</script><text>SVG text</text></svg><p>After SVG.</p>')
    expect(result).not_to be_empty
