...
<!-- alef:hash:dc8f156e4306648e44ab4736b7d73212d8faf8f9a2a212e4cb52926f5a37f3ea -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## R API Reference <span class="version-badge">v3.15.0</span>
...
| `tables` | `list` | `list()` | Extracted tables with structured cell data and markdown representation. Table data is collected by the same pass that builds `Self.document`, so it is populated only when the `include_document_structure` option is `true`. With the default options this is an empty list even for input that contains tables — the tables still appear in `Self.content` as rendered Markdown. |
...
| `custom` | Replace default output with custom markdown The visitor takes full responsibility for the markdown output of this node and its children. — Fields: `output`: `character` |
...
| `error` | Stop conversion with an error The conversion process halts and returns this error message. — Fields: `output`: `character` |
