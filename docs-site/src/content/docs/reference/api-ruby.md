...
<!-- alef:hash:cee9569a1f71ab2dfea965da5e57b5168b305e653560ac212a74ae210e6a748c -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Ruby API Reference <span class="version-badge">v3.15.0</span>
...
| `tables` | `Array<TableData>` | `[]` | Extracted tables with structured cell data and markdown representation. Table data is collected by the same pass that builds `Self.document`, so it is populated only when the `include_document_structure` option is `true`. With the default options this is an empty list even for input that contains tables — the tables still appear in `Self.content` as rendered Markdown. |
