...
<!-- alef:hash:127bb740ed514d71704bdb8203cde6df31010971c78bace5e88fff190fac860c -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Java API Reference <span class="version-badge">v3.15.0</span>
...
| `options` | `@Nullable ConversionOptions` | No | The options to use |

**Returns:** `ConversionResult`

**Errors:** Throws `HtmlToMarkdownRsException`.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `@Nullable String` | `null` | Converted text output in the selected format: Markdown, Djot, or plain text. |
| `document` | `@Nullable DocumentStructure` | `null` | Structured document tree with semantic elements. Populated when the `include_document_structure` option is `true`. `null` otherwise (the default), which avoids the overhead of building the tree. When present, the tree mirrors the converted document: headings open `Group` sections, paragraphs and list items carry inline `TextAnnotation`s, and tables reference the same `TableGrid` data exposed in the result's `tables` field. Note: this field is independent of the `metadata` feature flag. Document structure collection is always available at runtime; it is gated only by the runtime option, not by a compile-time feature. |
| `metadata` | `HtmlMetadata` | — | Extracted HTML metadata (title, OG, links, images, structured data). |
| `tables` | `List<TableData>` | `Collections.emptyList()` | Extracted tables with structured cell data and markdown representation. Table data is collected by the same pass that builds `Self.document`, so it is populated only when the `include_document_structure` option is `true`. With the default options this is an empty list even for input that contains tables — the tables still appear in `Self.content` as rendered Markdown. |
...
| `parent` | `@Nullable Integer` | `null` | Index of the parent node (None for root nodes). |
...
| `sourceFormat` | `@Nullable String` | `null` | The source format (always "html" for this library). |
...
| `rowSpan` | `int` | `1` | Number of rows this cell spans (default 1). |
| `colSpan` | `int` | `1` | Number of columns this cell spans (default 1). |
...
```java
public VisitResult visitText(NodeContext ctx, String text) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitElementStart(NodeContext ctx) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitElementEnd(NodeContext ctx, String output) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitLink(NodeContext ctx, String href, String text, String title) throws HtmlToMarkdownRsException
```
...
| `text` | `String` | Yes | The  text |
| `title` | `@Nullable String` | No | The  title |
...
```java
public VisitResult visitImage(NodeContext ctx, String src, String alt, String title) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitHeading(NodeContext ctx, int level, String text, String id) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitCodeBlock(NodeContext ctx, String lang, String code) throws HtmlToMarkdownRsException
```
...
|------|------|----------|-------------|
| `ctx` | `NodeContext` | Yes | The node context |
| `lang` | `@Nullable String` | No | The  lang |
...
```java
public VisitResult visitCodeInline(NodeContext ctx, String code) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitListItem(NodeContext ctx, boolean ordered, String marker, String text) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitListStart(NodeContext ctx, boolean ordered) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitListEnd(NodeContext ctx, boolean ordered, String output) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitTableStart(NodeContext ctx) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitTableRow(NodeContext ctx, List<String> cells, boolean isHeader) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitTableEnd(NodeContext ctx, String output) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitBlockquote(NodeContext ctx, String content, long depth) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitStrong(NodeContext ctx, String text) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitEmphasis(NodeContext ctx, String text) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitStrikethrough(NodeContext ctx, String text) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitUnderline(NodeContext ctx, String text) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitSubscript(NodeContext ctx, String text) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitSuperscript(NodeContext ctx, String text) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitMark(NodeContext ctx, String text) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitLineBreak(NodeContext ctx) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitHorizontalRule(NodeContext ctx) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitCustomElement(NodeContext ctx, String tagName, String html) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitDefinitionListStart(NodeContext ctx) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitDefinitionTerm(NodeContext ctx, String text) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitDefinitionDescription(NodeContext ctx, String text) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitDefinitionListEnd(NodeContext ctx, String output) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitForm(NodeContext ctx, String action, String method) throws HtmlToMarkdownRsException
```
...
|------|------|----------|-------------|
| `ctx` | `NodeContext` | Yes | The node context |
| `action` | `@Nullable String` | No | The  action |
| `method` | `@Nullable String` | No | The  method |
...
```java
public VisitResult visitInput(NodeContext ctx, String inputType, String name, String value) throws HtmlToMarkdownRsException
```
...
| `name` | `@Nullable String` | No | The  name |
| `value` | `@Nullable String` | No | The  value |
...
```java
public VisitResult visitButton(NodeContext ctx, String text) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitAudio(NodeContext ctx, String src) throws HtmlToMarkdownRsException
```
...
|------|------|----------|-------------|
| `ctx` | `NodeContext` | Yes | The node context |
| `src` | `@Nullable String` | No | The  src |
...
```java
public VisitResult visitVideo(NodeContext ctx, String src) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitIframe(NodeContext ctx, String src) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitDetails(NodeContext ctx, boolean open) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitSummary(NodeContext ctx, String text) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitFigureStart(NodeContext ctx) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitFigcaption(NodeContext ctx, String text) throws HtmlToMarkdownRsException
```
...
```java
public VisitResult visitFigureEnd(NodeContext ctx, String output) throws HtmlToMarkdownRsException
```
...
| `alt` | `@Nullable String` | `null` | Alternative text from alt attribute (for accessibility) |
| `title` | `@Nullable String` | `null` | Title attribute (often shown as tooltip) |
| `dimensions` | `@Nullable ImageDimensions` | `null` | Image dimensions in pixels, if available. |
...
| `title` | `@Nullable String` | `null` | Optional title attribute (often shown as tooltip) |
...
| `parentTag` | `@Nullable String` | `null` | Parent element's tag name (None if root) |
...
```java
public void close() throws HtmlToMarkdownRsException
```
...
| `CUSTOM` | Replace default output with custom markdown The visitor takes full responsibility for the markdown output of this node and its children. — Fields: `output`: `String` |
...
| `ERROR` | Stop conversion with an error The conversion process halts and returns this error message. — Fields: `output`: `String` |
...
| Variant | Description |
|---------|-------------|
| `ParseErrorException` | HTML parsing error |
| `SanitizationErrorException` | HTML sanitization error |
| `ConfigErrorException` | Invalid configuration |
| `IoErrorException` | I/O error — stores the error message string so the variant is FFI-safe. Use `ConversionError.from(io_error)` to convert from an operating-system I/O error. |
| `PanicException` | Internal error caught during conversion |
| `InvalidInputException` | Invalid input data |
| `OtherException` | Generic conversion error |
