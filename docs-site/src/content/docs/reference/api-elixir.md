...
<!-- alef:hash:9159845272438609bd85ea10e7b91637fc1b7f1a0e5c68f280db25e84dc6a2ac -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Elixir API Reference <span class="version-badge">v3.15.0</span>
...
| `tables` | `list(TableData)` | `[]` | Extracted tables with structured cell data and markdown representation. Table data is collected by the same pass that builds `Self.document`, so it is populated only when the `include_document_structure` option is `true`. With the default options this is an empty list even for input that contains tables — the tables still appear in `Self.content` as rendered Markdown. |
...
| `row_span` | `integer()` | `1` | Number of rows this cell spans (default 1). |
| `col_span` | `integer()` | `1` | Number of columns this cell spans (default 1). |
...
```elixir
def visit_element_start(obj, ctx)
```
...
```elixir
def visit_element_end(obj, ctx, output)
```
...
```elixir
def visit_link(obj, ctx, href, text, title)
```
...
```elixir
def visit_image(obj, ctx, src, alt, title)
```
...
```elixir
def visit_heading(obj, ctx, level, text, id)
```
...
```elixir
def visit_code_block(obj, ctx, lang, code)
```
...
```elixir
def visit_code_inline(obj, ctx, code)
```
...
```elixir
def visit_list_item(obj, ctx, ordered, marker, text)
```
...
```elixir
def visit_list_start(obj, ctx, ordered)
```
...
```elixir
def visit_list_end(obj, ctx, ordered, output)
```
...
```elixir
def visit_table_row(obj, ctx, cells, is_header)
```
...
```elixir
def visit_table_end(obj, ctx, output)
```
...
```elixir
def visit_blockquote(obj, ctx, content, depth)
```
...
```elixir
def visit_emphasis(obj, ctx, text)
```
...
```elixir
def visit_strikethrough(obj, ctx, text)
```
...
```elixir
def visit_underline(obj, ctx, text)
```
...
```elixir
def visit_subscript(obj, ctx, text)
```
...
```elixir
def visit_superscript(obj, ctx, text)
```
...
```elixir
def visit_horizontal_rule(obj, ctx)
```
...
```elixir
def visit_custom_element(obj, ctx, tag_name, html)
```
...
```elixir
def visit_definition_list_start(obj, ctx)
```
...
```elixir
def visit_definition_term(obj, ctx, text)
```
...
```elixir
def visit_definition_description(obj, ctx, text)
```
...
```elixir
def visit_definition_list_end(obj, ctx, output)
```
...
```elixir
def visit_form(obj, ctx, action, method)
```
...
```elixir
def visit_input(obj, ctx, input_type, name, value)
```
...
```elixir
def visit_details(obj, ctx, open)
```
...
```elixir
def visit_summary(obj, ctx, text)
```
...
```elixir
def visit_figure_start(obj, ctx)
```
...
```elixir
def visit_figcaption(obj, ctx, text)
```
...
```elixir
def visit_figure_end(obj, ctx, output)
```
...
| `custom` | Replace default output with custom markdown The visitor takes full responsibility for the markdown output of this node and its children. — Fields: `output`: `String.t()` |
...
| `error` | Stop conversion with an error The conversion process halts and returns this error message. — Fields: `output`: `String.t()` |
