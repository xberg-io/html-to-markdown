...
<!-- alef:hash:35ca9467ce66ee17c3a0aeb53cf59ce1f548e19af7671b402c8899748031d61a -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Swift API Reference <span class="version-badge">v3.15.0</span>
...
| `tables` | `\[TableData\]` | `[]` | Extracted tables with structured cell data and markdown representation. Table data is collected by the same pass that builds `Self.document`, so it is populated only when the `include_document_structure` option is `true`. With the default options this is an empty list even for input that contains tables — the tables still appear in `Self.content` as rendered Markdown. |
...
```swift
public func visitBlockquote(ctx: NodeContext, content: String, depth: UInt) -> VisitResult
```
