...
<!-- alef:hash:858fd7b3881db96921f8a115aaf0ac411b6b30759de86f62b052689c604bc988 -->
...
```csharp
var result = HtmlToMarkdownConverter.Convert("value", new ConversionOptions());
```
...
| `Tables` | `List<TableData>` | `new List<TableData>()` | Extracted tables with structured cell data and markdown representation. Table data is collected by the same pass that builds `Self.document`, so it is populated only when the `include_document_structure` option is `true`. With the default options this is an empty list even for input that contains tables — the tables still appear in `Self.content` as rendered Markdown. |
...
| `RowSpan` | `uint` | `1` | Number of rows this cell spans (default 1). |
| `ColSpan` | `uint` | `1` | Number of columns this cell spans (default 1). |
...
| `Depth` | `ulong` | — | Document tree depth at the header element |
| `HtmlOffset` | `ulong` | — | Byte offset in original HTML document |
...
| `Depth` | `ulong` | — | Depth in the DOM tree (0 = root) |
| `IndexInParent` | `ulong` | — | Index among siblings (0-based) |
